...
description: Commit, push, and create a GitHub pull request from local repository changes with safety checks, commit conventions, base branch detection, and branch guardrails. Use when Codex should deliver a full PR-ready Git workflow from working tree changes.
---
...
1. Load `/.codex/skills/_shared/commit-conventions.md` before writing commit/PR titles.
2. Apply commit type/context/title rules from that file.
3. Use Codex attribution in commit and PR output text.
...
if [ "$CURRENT_BRANCH" = "main" ] || [ "$CURRENT_BRANCH" = "alpha" ] || [ "$CURRENT_BRANCH" = "$BASE_BRANCH" ]; then
  NEW_BRANCH="codex/$(date +%Y%m%d-%H%M)-commit-pr"
