...
# Install only production dependencies for the runtime workspace
...
# Generate Prisma client inside the production image (needs schema + node_modules)
RUN pnpm --package prisma@6.16.3 dlx prisma generate \
...
RUN mkdir -p /app/uploads/lagekarte \
    && chown -R node:node /app/uploads /app/packages/backend
...
# Start NestJS backend (dist/src/main.js generated by nest build)
