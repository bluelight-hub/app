...
|Datenbank
|SQLite-Datenbank für die lokale Datenspeicherung. Wird im FüKW gehostet und speichert alle Einsatzdaten.
...
* **Datenbank-Setup**: Die SQLite-Datenbank wird lokal auf dem FüKW eingerichtet
...
flowchart TD
    ...
    subgraph FüKW_Network["FüKW Lokales Netzwerk (10.0.0.0/24)"]
        ...
        subgraph Server["FüKW Server (10.0.0.2)"]
            ...
            Database["SQLite Datenbank (lokal)"]
