...
Das System muss in lokalen Netzwerken (FüKW) funktionieren und optional erweiterte Funktionen bieten, wenn Internetverbindung besteht.

Es ist wichtig zu beachten, dass "Offline-First" in diesem Kontext bedeutet, dass der FüKW-Server nicht immer mit dem Internet verbunden ist, aber eine Verbindung der Clients zum FüKW-Server in der aktuellen Implementierung vorausgesetzt wird.
Eine vollständige Client-Autonomie ohne FüKW-Verbindung ist für zukünftige Versionen geplant.
