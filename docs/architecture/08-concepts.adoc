...
* `X-Content-Type-Options: nosniff` - Verhindert MIME-Type-Sniffing * `X-Frame-Options: DENY` - Schutz vor Clickjacking * `X-XSS-Protection: 1; mode=block` - XSS-Schutz (Legacy-Browser) * `Referrer-Policy: strict-origin-when-cross-origin` - Kontrollierte Referrer-Informationen * `Permissions-Policy` - Einschränkung von Browser-Features
...
* **Versionierung**: URI-basierte Versionierung (`/api/v-1/...`) für maximale Kompatibilität
