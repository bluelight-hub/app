...
Das Frontend des Bluelight-Hub-Systems basiert auf einer modernen, komponentenbasierten Architektur mit Fokus auf Performance, Wartbarkeit und Developer Experience. Die Architektur nutzt das Atomic Design Pattern in Kombination mit einem Utility-First CSS-Ansatz.
...
Die zentrale Theme-Konfiguration erfolgt in Tailwind CSS v4 direkt über CSS mit dem `@theme` Directive in `index.css`:
