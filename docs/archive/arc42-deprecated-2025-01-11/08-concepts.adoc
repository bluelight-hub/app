...
* Einfachere Implementierung und Wartung * Bessere Verständlichkeit für neue Entwickler * Ausreichende Trennung der Zuständigkeiten * Skalierbarkeit für die aktuelle Anwendungsgröße
...
==== No-Delete Policy für Einsätze

Das Bluelight-Hub-System implementiert eine strikte No-Delete Policy für Einsätze aus rechtlichen und Compliance-Gründen.

Siehe auch <<../adr/017-no-delete-policy-einsaetze.adoc#,ADR-017: No-Delete Policy für Einsätze>>.

===== Rechtliche Anforderungen

Einsatzdaten unterliegen besonderen rechtlichen Aufbewahrungspflichten:

* *Gesetzliche Aufbewahrungsfristen*: Einsatzdokumentationen müssen gemäß relevanten Vorschriften (u.a. Datenschutz, Haftungsrecht) für mindestens 10 Jahre aufbewahrt werden
* *Audit-Trail*: Vollständige Nachvollziehbarkeit aller Einsatzaktivitäten für rechtliche Überprüfungen
* *Beweissicherung*: Einsatzdaten können als Beweismittel in rechtlichen Verfahren dienen
* *Compliance*: Erfüllung regulatorischer Anforderungen für Rettungsorganisationen

===== Technische Implementierung

Die No-Delete Policy wird auf mehreren Ebenen durchgesetzt:

[source,typescript]
----
// Beispiel: EinsatzController - KEIN Delete-Endpoint vorhanden
@Controller('einsatz')
export class EinsatzController {
  ...
  // ❌ VERBOTEN: Delete-Operationen
  // @Delete(':id') - NIEMALS implementieren!
}
----

===== Archivierungs-Strategie

Anstelle physischer Löschungen nutzt das System einen Archivierungs-Ansatz:

* *Status ARCHIVIERT*: Einsätze werden als "archiviert" markiert statt gelöscht
* *Soft-Delete Pattern*: Datensätze bleiben in der Datenbank erhalten
* *Filterung*: Archivierte Einsätze werden standardmäßig in Abfragen ausgeblendet
...
enum EinsatzStatus {
  ...
  ARCHIVIERT = 'ARCHIVIERT'  // Statt physischer Löschung
}
----

===== Sicherheitsmaßnahmen

Zusätzliche Schutzmaßnahmen gegen versehentliche Löschungen:

* *Keine DELETE-Endpoints*: API bietet keine Lösch-Funktionalität
* *Datenbank-Constraints*: Foreign Key Constraints mit RESTRICT/NO ACTION
* *Berechtigungssystem*: Selbst Super-Admins können Einsätze nicht löschen
* *Code-Reviews*: Automatische Überprüfung auf DELETE-Operationen in CI/CD

===== Ausnahmen und Sonderfälle

Die einzigen Szenarien für Datenentfernung:

* *Anonymisierung*: Personenbezogene Daten nach DSGVO-Anforderung
* *Datenbank-Migration*: Nur mit expliziter Genehmigung und Backup
* *Testdaten*: Nur in Entwicklungs- und Testumgebungen

Diese Policy stellt sicher, dass alle Einsatzdaten für rechtliche, operative und historische Zwecke dauerhaft verfügbar bleiben.
...
=== Frontend-Architektur und Component-System

Das Frontend des Bluelight-Hub-Systems basiert auf einer modernen, komponentenbasierten Architektur mit Fokus auf Performance, Wartbarkeit und Developer Experience.
Die Architektur nutzt das Atomic Design Pattern in Kombination mit einem Utility-First CSS-Ansatz.
...
|Styling |Tailwind CSS |Utility-First CSS ohne Runtime-Overhead
|Components |Headless UI |Accessible, unstyled UI-Komponenten
|State |TanStack Store |Globales State Management
|Forms |TanStack Form + Zod |Type-safe Form Handling mit Validation
|Routing |TanStack Router |Type-safe Routing mit File-based Routes
|API |TanStack Query |Data Fetching und Caching
|Build |Vite |Schnelles HMR und optimierte Builds
|===

==== Atomic Design Pattern

Das Frontend folgt dem Atomic Design Pattern für eine strukturierte und skalierbare Component-Hierarchie:
...
graph TD
    ...
    A --> |Button, Input, Text| Base[Base Components]
    M --> |PasswordInput, Tabs| Composite[Composite Components]
    O --> |LoginWindow, UsersTable| Complex[Complex Modules]
    T --> |DashboardLayout| Layouts[Page Layouts]
    P --> |AdminPage, Dashboard| Routes[Route Components]
....
...
1. **Single Responsibility**: Jede Komponente hat eine klar definierte Aufgabe
2. **Composition over Inheritance**: Komponenten werden durch Komposition aufgebaut
3. **Props over State**: State wird minimal gehalten und nach oben delegiert
4. **Type Safety**: Vollständige TypeScript-Typisierung für alle Props und Events
5. **Named Exports**: Alle Komponenten verwenden Named Exports statt Default Exports für bessere Refactoring-Unterstützung und explizite Imports

==== Tailwind CSS Integration

Das Styling basiert vollständig auf Tailwind CSS für optimale Performance und Wartbarkeit:

===== Utility-First Ansatz

[source,tsx]
----
// Statt CSS-in-JS oder externe Stylesheets
...
Alle React-Komponenten im Projekt verwenden **Named Exports** für konsistente und explizite Imports:
...
// ❌ FALSCH: Default Export (nicht verwenden!)
export default function Button({ children, ...props }) {
  return <button {...props}>{children}</button>;
}

// Import-Beispiel mit Named Export:
...
**Vorteile der Named Exports:**
- Explizite und eindeutige Imports - Bessere IDE-Unterstützung für Refactoring und Auto-Imports - Konsistente Import-Statements über das gesamte Projekt - Möglichkeit mehrerer Exports aus einer Datei (z.B. Komponente + Utility-Funktionen)

===== Theme-Konfiguration

Die zentrale Theme-Konfiguration erfolgt in Tailwind CSS v4 direkt über CSS mit dem `@theme` Directive in `index.tailwind.css`:
...
Für komplexe, accessible UI-Patterns nutzen wir Headless UI:
...
* **Focus Management**: Automatisches Focus-Trapping
* **Screen Reader Support**: ARIA-Attribute voronfiguriert

==== Performance-Optimierung

===== Bundle-Size-Optimierung

* **Tree-Shaking**: Nur verwendete Utilities werden gebündelt
* **PurgeCSS**: Automatische Entfernung ungenutzter Styles
* **Code-Splitting**: Route-basiertes Lazy Loading
* **Zero Runtime**: Keine CSS-in-JS Runtime-Overhead
...
Globaler State wird mit TanStack Store verwaltet:
...
it('should not have accessibility violations', async () => {
  const { container } = render(<LoginWindow />);
  const results = await axe(container);
  expect(results).toHaveNoViolations();
});
----

==== Migration von Chakra UI

Die Migration von Chakra UI zu Tailwind CSS + Headless UI wurde aus folgenden Gründen durchgeführt:

1. **Performance**: Elimination der CSS-in-JS Runtime
2. **Bundle-Size**: Reduktion um ~60%
3. **Developer Experience**: Schnelleres HMR und bessere IDE-Unterstützung
4. **Wartbarkeit**: Weniger Dependencies und einfachere Updates

Details zur Migration sind in <<../adr/013-tailwind-migration.adoc>> dokumentiert.

==== Best Practices

1. **Component Composition**: Kleine, wiederverwendbare Komponenten
2. **Prop Types**: Vollständige TypeScript-Typisierung
3. **Accessibility First**: ARIA-Attribute und Keyboard-Support
4. **Performance**: Memoization und Code-Splitting wo sinnvoll
5. **Testing**: Unit- und Integration-Tests für kritische Komponenten

=== Einsatz-Management-System (Epic-143)

Das Einsatz-Management-System implementiert ein flexibles Konzept für die Verwaltung von Einsätzen ohne Pflichtparameter.
Dies ermöglicht eine schnelle Einsatzerstellung und schrittweise Vervollständigung der Daten während des laufenden Einsatzes.

==== Kernkonzepte

===== Minimale Einsatzerstellung

Einsätze können ohne jegliche Pflichtfelder angelegt werden. Das System generiert automatisch einen sinnvollen Namen aus den verfügbaren Informationen:

[source,typescript]
----
// Name-Generierung Hierarchie
interface NameGenerationStrategy {
  priority: number;
  generate: (einsatz: Partial<Einsatz>) => string | null;
}

const strategies = [
  { // Priorität 1: Alarmstichwort + Zeit
    generate: (e) => e.alarmstichwort && e.alarmierungszeit
      ? `${e.alarmstichwort} - ${formatDateTime(e.alarmierungszeit)}`
      : null
  },
  { // Priorität 2: Nur Alarmstichwort
    generate: (e) => e.alarmstichwort
      ? `${e.alarmstichwort} - ${formatDate(new Date())}`
      : null
  },
  { // Priorität 3: Nur Zeit
    generate: (e) => e.alarmierungszeit
      ? `Einsatz vom ${formatDateTime(e.alarmierungszeit)}`
      : null
  },
  { // Fallback: Aktuelles Datum/Zeit
    generate: () => `Einsatz vom ${formatDateTime(new Date())}`
  }
];
----
...
Das System berechnet kontinuierlich die Vollständigkeit jedes Einsatzes und gibt Hinweise auf fehlende Informationen:

[mermaid]
....
graph LR
    subgraph "Vollständigkeits-Score"
        Check[Prüfung aller Felder]
        Check --> Critical[Kritische Felder: 40%]
        Check --> Important[Wichtige Felder: 30%]
        Check --> Optional[Optionale Felder: 30%]
        Critical --> Score[Gesamtscore 0-100%]
        Important --> Score
        Optional --> Score
    end
    
    Score --> Feedback[User Feedback]
    Feedback --> Visual[Visuelle Indikatoren]
    Feedback --> Hints[Verbesserungshinweise]
....
...
// Beispiel-Berechnung
const calculateCompleteness = (einsatz: Einsatz): EinsatzCompleteness => {
  const weights = {
    critical: 0.4,    // 40% Gewichtung
    important: 0.3,   // 30% Gewichtung
    optional: 0.3     // 30% Gewichtung
  };
  
  // Feldprüfung und Score-Berechnung
  const missingFields = checkMissingFields(einsatz);
  const score = calculateWeightedScore(missingFields, weights);
...
Alle UI-Interaktionen werden optimistisch aktualisiert für sofortiges Feedback:
...
sequenceDiagram
    ...
    Store->>UI: Sofortige UI-Aktualisierung
    UI->>API: Request senden
    API->>DB: Persistieren
    DB-->>API: Response
    
    alt Erfolg
        API-->>Store: Server-Daten
        Store-->>UI: Bestätigung
    else Fehler
        API-->>Store: Error
        Store-->>UI: Rollback
...
===== In-Memory Cache ohne Redis

Statt externer Cache-Systeme nutzt das System einen effizienten In-Memory Cache:
...
Name und Vollständigkeit werden niemals in der Datenbank gespeichert, sondern immer zur Laufzeit berechnet:

* **Vorteile**: Garantierte Konsistenz, keine Sync-Probleme, reduzierter Speicherbedarf
* **Performance**: Berechnung < 1ms durch effiziente Algorithmen
* **Caching**: Kurzzeit-Cache für häufig abgerufene Werte

==== Status-Management

Das System implementiert einen klaren Status-Lifecycle mit definierten Transitionen:

[mermaid]
....
stateDiagram-v2
    [*] --> ANGELEGT: Erstellen
    ANGELEGT --> IN_BEARBEITUNG: Starten
    IN_BEARBEITUNG --> ABGESCHLOSSEN: Beenden
    ABGESCHLOSSEN --> ARCHIVIERT: Archivieren
    
    IN_BEARBEITUNG --> ANGELEGT: Zurücksetzen
    ABGESCHLOSSEN --> IN_BEARBEITUNG: Wiedereröffnen
    
    note right of ARCHIVIERT: Keine Änderungen mehr möglich
....
...
Alle Endpoints folgen REST-Prinzipien und sind vollständig in OpenAPI dokumentiert:

[source,yaml]
----
/api/einsatz:
  post:
    summary: Einsatz erstellen (ohne Pflichtfelder)
    requestBody:
      required: false  # Kein Body erforderlich!
      content:
        application/json:
          schema:
            type: object
...
===== Generierte Type-Safe Clients

Frontend nutzt ausschließlich generierte API-Clients:
...
// IMMER generiert nutzen:
// ✅ import { EinsatzApi } from '@bluelight-hub/shared/client';
const api = new EinsatzApi();
const response = await api.createEinsatz({}); // Type-safe!
----

==== Architektur-Entscheidungen

Die wichtigsten Entscheidungen für das Einsatz-Management:

1. **Keine Pflichtfelder**: Maximale Flexibilität im Einsatz
2. **Computed Fields**: Konsistenz über Performance
3. **In-Memory Cache**: Einfachheit über Skalierung (für den Anwendungsfall ausreichend)
4. **Optimistic Updates**: User Experience über Datenkonsistenz (mit Rollback)
5. **Generierte API Clients**: Type-Safety über Flexibilität

== Entwicklungsdokumentation

Für detaillierte Entwicklungsrichtlinien siehe:

=== Coding Standards
Siehe link:../development/coding-standards.md[Coding Standards]

=== Technology Stack
Siehe link:../development/tech-stack.md[Technology Stack]

=== Projektstruktur
Siehe link:../development/project-structure.md[Projektstruktur]

=== Brownfield-Entwicklung
Siehe link:../development/brownfield-guide.md[Brownfield Guide]
