...
|Entwurfsmethodik für UI-Komponenten, die diese hierarchisch in Atoms (kleinste Bausteine), Molecules (einfache Kombinationen), Organisms (komplexe Komponenten), Templates (Seitenlayouts) und Pages (konkrete Instanzen) unterteilt.
...
|Gesamtgröße aller JavaScript-, CSS- und anderen Asset-Dateien, die an den Browser ausgeliefert werden. Kleinere Bundle Sizes führen zu schnelleren Ladezeiten und besserer Performance.
...
|[[css-in-js]]CSS-in-JS
|Ansatz, bei dem CSS-Styles direkt in JavaScript definiert werden. Wurde im Bluelight-Hub-System durch Tailwind CSS ersetzt, um Runtime-Overhead zu vermeiden und die Performance zu verbessern.
...
|Einsatz
|Organisierter Einsatz von Kräften und Mitteln zur Bewältigung einer Gefahr oder eines Notfalls; zentrale Entität im Bluelight-Hub-System. Kann ohne Pflichtparameter angelegt werden (siehe Epic-143).
...
|Standardisierte Bezeichnung für eine Einsatzart oder einen Alarmgrund (z.B. "Brand 3", "H1", "THL"). Optionales Feld bei der Einsatzerstellung.

|[[alarmierungszeit]]Alarmierungszeit
|Zeitpunkt, zu dem die Alarmierung der Einsatzkräfte erfolgte. Optionales Feld bei der Einsatzerstellung.

|[[computed-fields]]Computed Fields
|Dynamisch berechnete Felder, die niemals in der Datenbank gespeichert werden, sondern zur Laufzeit aus vorhandenen Daten generiert werden. Beispiele: Einsatzname und Vollständigkeitsscore.

|[[completeness-score]]Completeness Score
|Prozentuale Bewertung (0-100%) der Vollständigkeit eines Einsatzdatensatzes, berechnet aus kritischen (40%), wichtigen (30%) und optionalen (30%) Feldern.

|[[in-memory-cache]]In-Memory Cache
|Zwischenspeicher im Arbeitsspeicher für häufig abgerufene Berechnungen wie Vollständigkeitsanalysen, mit kurzer TTL (Time-to-Live, meist 60 Sekunden).
...
|Sammlung von vollständig ungestylten, aber voll funktionsfähigen und barrierefreien UI-Komponenten für React und Vue. Im Bluelight-Hub-System verwendet für komplexe, interaktive UI-Patterns wie Dialoge, Dropdowns und Tabs.

|[[hmr]]HMR
|Hot Module Replacement; Entwicklungsfunktion, die es ermöglicht, Module in einer laufenden Anwendung zu aktualisieren, ohne die gesamte Seite neu zu laden, was zu schnelleren Entwicklungszyklen führt.
...
|[[minimale-einsatzerstellung]]Minimale Einsatzerstellung
|Konzept aus Epic-143, bei dem Einsätze ohne jegliche Pflichtparameter angelegt werden können, um schnelle Erfassung in Stresssituationen zu ermöglichen.
...
|UI-Pattern, bei dem Benutzeraktionen sofort in der Oberfläche reflektiert werden, noch bevor die Server-Antwort eintrifft. Bei Fehlern erfolgt automatischer Rollback.
...
|Utility-First CSS-Framework, das vordefinierte Utility-Klassen bereitstellt, um direkt im HTML/JSX Styles zu definieren, ohne separate CSS-Dateien schreiben zu müssen. Ermöglicht schnelle Entwicklung und konsistentes Design.

|[[tanstack]]TanStack
|Sammlung von modernen React-Bibliotheken für State Management (Store), Routing (Router), Forms (Form), Data Fetching (Query) und mehr. Im Bluelight-Hub-System als Hauptbibliotheken für diese Bereiche verwendet.
...
|Optimierungstechnik beim Build-Prozess, die ungenutzten Code automatisch entfernt, um die finale Bundle Size zu reduzieren.
...
|CSS-Ansatz, bei dem kleine, einzweckige Utility-Klassen (wie "text-center", "bg-red-500") direkt im Markup verwendet werden, anstatt semantische CSS-Klassen mit vielen Eigenschaften zu schreiben.
