...
Das Modul **Eigenschutz** digitalisiert die Arbeitsschutz-Aufgaben des Sicherheitsbeauftragten im Stab – in Echtzeit, während des Einsatzes, nicht danach. Zielnutzer sind Sicherheitsbeauftragte (S-Stab), Einsatzabschnittsleiter, Einheitsführer und Atemschutz-Überwachungstrupps in BOS-Organisationen (Katastrophenschutz, Feuerwehr, Rettungsdienst, THW).
...
2. **Vorlagen-Bibliothek für Standardszenarien** (MANV, CBRN, Wald, Hochwasser, Verkehrsunfall). Der Sicherheitsbeauftragte startet nicht bei Null – er passt vorgedachte Gefährdungs-Sets an die konkrete Lage an.
...
**Hintergrund:** Markus, 42, ist Zugführer bei einem THW-Ortsverband und seit drei Jahren als Sicherheitsbeauftragter für den Katastrophenschutz qualifiziert. Bei Einsätzen wird er vom Einsatzleiter regelmäßig als S-Stab eingebunden. Er kennt die DGUV-Vorschriften, aber unter Zeitdruck passieren ihm Lücken in der Doku, die ihn nachts wach halten.
...
**Hintergrund:** Steffi, 35, Oberfeldwebel der Freiwilligen Feuerwehr, führt den Abschnitt „Verkehr" bei einer Unwetter-Lage. Sie hat fünf Einheiten unter sich. Für sie zählt: Keine Überraschungen, klare Ansagen zur PSA, sichtbarer Schutzstatus ihrer Leute.
...
- **Rising Action.** Tippt den Banner, sieht Checkliste der neuen Ausrüstung (CSA, umgebungsluftunabhängiger Atemschutz). Markiert je Einheit, ob Ausrüstung verfügbar ist. Bei einer Einheit fehlt die Ausrüstung — sie setzt einen Vermerk, der den Sicherheitsbeauftragten benachrichtigt.
...
**Hintergrund:** Lukas, 28, bei der Berufsfeuerwehr speziell für die Überwachung von Atemschutztrupps zuständig.

#### Journey 5 — Atemschutz-Trupp-Überwachung (Post-MVP)

- **Opening Scene.** Wohnungsbrand, zwei Trupps gehen unter Atemschutz in den Innenangriff.
- **Rising Action.** Lukas legt pro Trupp einen Eintrag an: Mitglieder, Startdruck, Einsatzbeginn. Das System startet einen Timer und warnt bei kritischer Einsatzzeit/-druck.
- **Climax.** Bei Zeit-Schwellwert blinkt der Trupp-Eintrag, parallel erhält die Funk-Stelle einen Hinweis. Lukas quittiert den Rückzug, trägt Endwerte ein.
- **Resolution.** Atemschutzprotokoll ist vollständig, verknüpft mit dem Einsatz und mit dem Eigenschutz-Modul.
- **Revealed Capabilities.** Trupp-Management, Timer-basierte Warnungen, Integration mit Funk-/Kommunikations-Sichten. (Growth-Scope: bewusst nicht im MVP.)
...
| Atemschutz-Überwachung (Growth) | 5 |
...
- **Landesrechtliche Besonderheiten** (Feuerwehrgesetze, Katastrophenschutzgesetze der Länder): Nicht im MVP hart kodiert; Landesunfallkassen-Exportformate als Growth-Feature.
...
- **Piloteinsatz** in 1–2 befreundeten Organisationen (THW-Ortsverband, Freiwillige Feuerwehr) während Übungen — Messpunkte: Erfassungszeit, Fehlerquote Unfallkassen-Export, Akzeptanz durch S-Stab.
...
- MVP muss die Journeys 1a, 1b, 2 und 4 vollständig tragen. Journey 3 (Admin/Template) im MVP in **minimaler Form** (Basis-Vorlagen werden ausgeliefert, keine vollwertige Vorlagen-Versionierungs-UI). Journey 5 (Atemschutz) ausschließlich Post-MVP.
...
**Core Journeys im MVP:** 1a, 1b, 2, 4 (vollständig); 3 als Basis-Seeding; 5 ausgeklammert.
...
- Standardszenarien-Pack (MANV, CBRN, Wald, Hochwasser, Verkehr)
...
- Atemschutz-Überwachung (Journey 5)
...
- *Risiko:* Offline-Sync-Komplexität bei Multi-Device-Parallelnutzung. *Mitigation:* Konsequente Nutzung der Platform-Storage-Strategie (ADR-010), keine Eigenentwicklung; Event-basierter Merge-Pfad; kritische Felder (PSA-Stufe) mit expliziter Konfliktmarkierung statt automatischem Überschreiben.
- *Risiko:* Lagekarte-Integration aufwändiger als erwartet (MapGL-spezifisch). *Mitigation:* Sicherungsposten-Marker als minimaler Feature-Cut im MVP; komplexere Karteninteraktionen Phase 2.
- *Risiko:* Echtzeit-Propagation skaliert nicht auf viele Clients. *Mitigation:* Bestehenden Event-Kanal benchmarken vor Modulentwicklung; Fallback-Pfad Polling-Backoff.

**Markt-/Anwendungs-Risiken:**

- *Risiko:* Modul wird als „noch ein Papier-Ersatz" empfunden und nicht genutzt. *Mitigation:* Piloteinsatz in 1–2 befreundeten Organisationen mit definierten Messgrößen; Ampel-Dashboard und Tempo-Garantien (< 2 min Gefährdungsbeurteilung) als Hauptargument; Post-Pilot-Interview-Runde als Kurs-Korrektur-Gate.
- *Risiko:* Unfallkassen-Formate abweichend pro Bundesland. *Mitigation:* MVP liefert generischen Export; Formatspezifik Phase 2 nach konkreter Pilot-Organisation.

**Ressourcen-Risiken:**

- *Risiko:* Team-Kapazität kleiner als geplant. *Mitigation:* Innerhalb des MVP-Featuresets Sub-Schnitte möglich — minimaler lauffähiger Kern (mMVP) wären Journeys 1a + 2 + 4 ohne Sicherungsposten und ohne Ampel-Dashboard. Ampel und Sicherungsposten als erste streichbare Features, wenn Kapazität eng.
- *Risiko:* Externe Rechts-/Sicherheits-Konsultation verzögert sich. *Mitigation:* Im MVP-Scope ist Entwurf & Implementierung von externer Freigabe unabhängig; Konsultation parallel; Release-Gate bleibt an Pilot-Akzeptanz, nicht an formaler Freigabe.

## Functional Requirements

Diese FR-Liste ist der **Capability-Contract** des Moduls: alles, was hier fehlt, existiert am Ende nicht. FRs beschreiben *was* das System kann, nicht *wie* es implementiert ist. Scope-Marker: `[MVP]`, `[Phase 2]`, `[Phase 3]`.
...
- **FR22** `[Phase 2]` Das System kann Push-Notifications (Web-Push / Desktop) für kritische Ereignisse senden.
...
- **Vollschutz (Grenzbereich)** — gasdichter CSA + umluftunabhängiger Atemschutz. Für weiße BOS **nicht eigenständig** — üblicherweise Übergabe an Feuerwehr/ABC-Zug. Relevant nur für Medizinische Task Force (MTF).
...
### Q4 — Rollen-Modell ✅ **Hybrid-Ansatz**
...
**Entscheidung:** Hybrid, minimal-invasiv.
...
- `packages/backend/prisma/schema.prisma` — neuer Enum + ggf. Verknüpfung zu `EinsatzRollenbesetzung`
- `packages/backend/src/modules/auth/guards/` — ggf. `EigenschutzRolleGuard` oder Wiederverwendung bestehender Guards mit Permission-Checks
...
### Q7 — Unfallkassen-Export-Schema ⏳ **MVP-Entwurf akzeptiert, iterativ**

**Entscheidung:** MVP liefert den im PRD vorgeschlagenen Pflicht-Feld-Satz als Obermenge (siehe Scoping-Abschnitt, Persona 4 / Journey 4 und FR31–FR37). Keine landesunfallkassen-spezifische Anpassung im MVP. Konkretes Schema wird in der Architecture-Phase als TypeScript-Interface + Zod-Schema finalisiert.
...
- **A6:** **Atemschutz-Überwachung ist explizit Phase 2**, selbst wenn Feuerwehr-Piloten sie früh wünschen — MVP-Scope wird dafür nicht aufgeweicht.
