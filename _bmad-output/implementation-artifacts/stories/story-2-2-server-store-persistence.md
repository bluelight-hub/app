...
- Hook: `useLoadServers()` triggert `hydrateServerStore()` in `useEffect` (nur einmal beim App-Start)
...
- [ ] **Subtask 1.1:** TypeScript Types definieren in `features/server/types/server-config.ts`
  - [ ] Interface `ServerConfig` mit allen Feldern (id, name, url, accessToken?, isDefault, createdAt, lastUsedAt)
  - [ ] Type `ConnectionStatus = 'connected' | 'disconnected' | 'checking'`
  - [ ] Interface `ServerState` für Store State (servers, activeServerId, connectionStatus, isHydrated)
  - [ ] JSDoc Kommentare (Deutsch) für alle public Types
- [ ] **Subtask 1.2:** TanStack Store erstellen in `features/server/stores/server.store.ts`
  - [ ] `export const serverStore = new Store<ServerState>(initialState)`
  - [ ] Initial State: `{ servers: [], activeServerId: null, connectionStatus: new Map(), isHydrated: false }`
  - [ ] Keine Actions im Store selbst (Actions sind externe Funktionen!)
  - [ ] Export Store für Hook-Nutzung
- [ ] **Subtask 1.3:** Storage Helper in `features/server/stores/server-persistence.ts`
  - [ ] `saveServers(servers: ServerConfig[]): Promise<void>` - schreibt zu Storage Adapter
  - [ ] `loadServers(): Promise<ServerConfig[]>` - liest von Storage Adapter
  - [ ] Storage-Key Konstante: `export const STORAGE_KEY_SERVERS = 'bluelight:servers'`
  - [ ] Error Handling: Try-Catch mit Console-Warning bei Fehler

### Task 2: Server CRUD Actions (AC1, AC2, AC4) ✅

- [ ] **Subtask 2.1:** Add Server Action in `features/server/stores/server.store.ts`
  - [ ] `export async function addServer(config: Omit<ServerConfig, 'id' | 'createdAt'>): Promise<void>`
  - [ ] Auto-generiere `id` (UUID v4 via `crypto.randomUUID()`)
  - [ ] Auto-setze `createdAt` (ISO String via `new Date().toISOString()`)
  - [ ] Validierung: URL Format, name min 1 char
  - [ ] Update Store State + async Storage Sync
  - [ ] Performance: Gesamtdauer < 200ms (NFR-P5)
- [ ] **Subtask 2.2:** Set Active Server Action
  - [ ] `export async function setActiveServer(serverId: string): Promise<void>`
  - [ ] Update `activeServerId` im Store
  - [ ] Update `isDefault: true` für aktiven Server, `false` für andere
  - [ ] Update `lastUsedAt` Timestamp für aktiven Server
  - [ ] Sync zu Storage Adapter
- [ ] **Subtask 2.3:** Remove Server Action
  - [ ] `export async function removeServer(serverId: string): Promise<void>`
  - [ ] Filter Server aus Array
  - [ ] Auto-Fallback: Wenn aktiver Server gelöscht → ersten Server aktiv setzen (oder null)
  - [ ] Sync zu Storage Adapter
  - [ ] Optional: Validation "min 1 Server" (siehe Dev Notes)

### Task 3: Hydration & Initialization (AC3) ✅

- [ ] **Subtask 3.1:** Load Servers Action
  - [ ] `export async function hydrateServerStore(): Promise<void>`
  - [ ] Read from Storage Adapter via `loadServers()`
  - [ ] Update Store State mit geladenen Servern
  - [ ] Setze `isHydrated: true` Flag
  - [ ] Auto-setze Default-Server als aktiv (find `isDefault: true`)
  - [ ] Error Handling: Bei Storage-Error bleibt Store leer (Console-Warning)
- [ ] **Subtask 3.2:** Custom Hook `useLoadServers` in `features/server/hooks/use-load-servers.ts`
  - [ ] Hook triggert `hydrateServerStore()` in `useEffect` (nur einmal)
  - [ ] Check `isHydrated` Flag um doppelte Hydration zu verhindern
  - [ ] Return `{ isLoading, error }` State für UI Feedback

### Task 4: Connection Status (AC5) ✅

- [ ] **Subtask 4.1:** Connection Status Action
  - [ ] `export function updateConnectionStatus(serverId: string, status: ConnectionStatus): void`
  - [ ] Update `connectionStatus` Map im Store (synchron!)
  - [ ] KEINE Persistierung (nur In-Memory)
- [ ] **Subtask 4.2:** Custom Hook `useConnectionStatus` in `features/server/hooks/use-connection-status.ts`
  - [ ] `export function useConnectionStatus(serverId: string): ConnectionStatus | undefined`
  - [ ] Nutzt `useStore(serverStore, (state) => state.connectionStatus.get(serverId))`
  - [ ] Default: `undefined` wenn Server nicht in Map

### Task 5: Custom Hooks für UI Integration ✅

- [ ] **Subtask 5.1:** `useActiveServer` Hook in `features/server/hooks/use-active-server.ts`
  - [ ] `export function useActiveServer(): ServerConfig | null`
  - [ ] Selector: `(state) => state.servers.find(s => s.id === state.activeServerId) ?? null`
  - [ ] Optimierte Performance durch Selector (kein unnötiges Re-Render)
- [ ] **Subtask 5.2:** `useServerList` Hook in `features/server/hooks/use-server-list.ts`
  - [ ] `export function useServerList(): ServerConfig[]`
  - [ ] Selector: `(state) => state.servers`
  - [ ] Optional: Sortierung nach `lastUsedAt` (neueste zuerst)
- [ ] **Subtask 5.3:** `useServerStore` Hook in `features/server/hooks/use-server-store.ts`
  - [ ] Wrapper Hook mit allen Actions als Return (ähnlich wie `useEinsatzStore`)
  - [ ] Return: `{ servers, activeServerId, addServer, setActiveServer, removeServer, ... }`
  - [ ] Nutzt `useStore(serverStore)` intern

### Task 6: Unit Tests (PFLICHT!) ✅

- [ ] **Subtask 6.1:** Store Tests in `features/server/stores/__tests__/server.store.test.ts`
  - [ ] Test: Add Server → Store aktualisiert, Storage synced
  - [ ] Test: Set Active Server → activeServerId + isDefault korrekt
  - [ ] Test: Remove Server → Server entfernt, Fallback aktiv
  - [ ] Test: Hydration → Storage loaded, Default-Server aktiv
  - [ ] Test: Connection Status → Map korrekt aktualisiert
  - [ ] Mock: `getStorageAdapter()` mit `vi.mock()`
- [ ] **Subtask 6.2:** Hook Tests in `features/server/hooks/__tests__/`
  - [ ] Test: `useActiveServer` → liefert aktiven Server oder null
  - [ ] Test: `useServerList` → liefert alle Server
  - [ ] Test: `useConnectionStatus` → liefert Status oder undefined
  - [ ] Test: `useLoadServers` → triggert Hydration nur einmal
  - [ ] Nutze `@testing-library/react-hooks` oder `renderHook` von Vitest
...
**Task Status:**
- [ ] Task 1: Store Definition & Types
- [ ] Task 2: Server CRUD Actions
- [ ] Task 3: Hydration & Initialization
- [ ] Task 4: Connection Status
- [ ] Task 5: Custom Hooks für UI Integration
- [ ] Task 6: Unit Tests

**Metrics (wird nach Completion gefüllt):**
- Test Count: TBD (Target: min 14 Tests)
- Test Coverage: TBD (Target: >80%)
- TypeScript Errors: 0 (Target)
- Biome Lint Errors: 0 (Target)
- Performance: TBD (Target: <200ms)

### File List

**Created/Modified Files (wird nach Completion gefüllt):**
