...
export function ConfirmationPrompt({ message, confirmLabel, cancelLabel, onConfirm, onCancel, disabled = false, intent = 'warning' }: ConfirmationPromptProps) {
  return (
    <div className="flex items-center gap-2 rounded-panel border border-status-warning-border bg-status-warning-surface px-3 py-1.5">
      <span className="text-sm text-status-warning-text">{message}</span>
