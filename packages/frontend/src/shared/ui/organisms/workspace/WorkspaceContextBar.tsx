    ...
    <header className={cn('sticky top-0 z-30 border-border-subtle border-b bg-surface-panel shadow-raised', className)}>
      <Container maxWidth="full">
        <div className="flex min-h-16 items-center justify-between gap-4 py-3">
          <div className="flex min-w-0 items-center gap-4">
            {backAction ? (
              <Link
                ...
                className={cn(
                  'group inline-flex items-center justify-center gap-2 rounded-control border border-transparent px-3 py-1.5 font-medium font-sans text-body-sm text-text-secondary transition-[background-color,border-color,color,box-shadow] hover:bg-action-secondary hover:text-text-primary focus:outline-none focus-visible:shadow-focus-ring',
                )}
              >
            ...
            {backAction ? <div className="h-8 w-px bg-border-subtle" aria-hidden="true" /> : null}

            <div className="flex min-w-0 items-center gap-3">
              {ContextIcon ? <ContextIcon className="h-5 w-5 flex-shrink-0 text-status-danger-text" aria-hidden="true" /> : null}
          ...
          {endSlot ? <div className="flex items-center gap-3">{endSlot}</div> : null}
        </div>
      </Container>
    </header>
  );
