...
export function FormFieldWrapper({ field, label, required = false, optional = false, helpText, className = '', children }: FormFieldWrapperProps) {
  return (
    <div className={className}>
      <label htmlFor={field.name} className="block font-medium text-sm text-text-secondary">
        {label}
        {required && <span className="ml-1 text-status-danger-text">*</span>}
        {optional && !required && <span className="ml-2 text-text-muted text-xs">(optional)</span>}
      </label>

      {helpText && <p className="mt-0.5 text-text-muted text-xs">{helpText}</p>}
