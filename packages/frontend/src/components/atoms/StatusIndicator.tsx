...
export const StatusIndicator: React.FC<StatusIndicatorProps> = ({
  ...
  const { getConnectionStatus, getSystemDetailsText, getDebugInfo } = useBackendHealth();
  ...
  return (
    <Popover content={popoverContent} title="Verbindungsstatus" trigger="hover" placement="right">
      <div className="flex items-center gap-x-2">
        <Badge status={badgeStatus} className={className} data-testid={dataTestId} />
        <p className="text-sm text-gray-500 dark:text-gray-400">{withText && text}</p>
      </div>
    </Popover>
  );
};
