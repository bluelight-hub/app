    ...
    Table: ({ dataSource, columns, onRow, loading, pagination, locale }: any) => {
      ...
      return (
        <div>
          ...
          {locale?.emptyText && dataSource?.length === 0 && <div>{locale.emptyText}</div>}
          <table>
            <thead>
              <tr>
                {columns.map((col: any) => (
            ...
            <tbody>
              {dataSource?.map((row: any) => {
                ...
                return (
                  <tr key={row.id} onClick={rowProps.onClick} style={rowProps.style} data-testid={`row-${row.id}`}>
                    {columns.map((col: any) => (
          ...
          {pagination && (
            <div>
              ...
              {showTotalText && <div data-testid="pagination-info">{showTotalText}</div>}
            </div>
          )}
        </div>
      );
