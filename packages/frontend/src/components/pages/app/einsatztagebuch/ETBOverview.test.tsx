...
vi.mock('@organisms/etb/ETBTable', () => ({
  ETBTable: ({
    ...
    <div data-testid="mock-etb-table">
      ...
      <button data-testid="archivieren-button" onClick={() => onArchiveEntry && onArchiveEntry(1)}>
        Archivieren
      </button>
    </div>
  ),
...
vi.mock('@organisms/etb/ETBCardList', () => ({
  ETBCardList: ({
    ...
    <div data-testid="mock-etb-cardlist">
      ...
      <button data-testid="mobile-archivieren-button" onClick={() => onArchiveEntry && onArchiveEntry(1)}>
        Archivieren Mobile
      </button>
    </div>
  ),
...
vi.mock('@organisms/etb/ETBEntryForm', () => ({
  ETBEntryForm: ({
    ...
    <div data-testid="mock-etb-entry-form">
      ...
      {editingEntry && <div data-testid="editing-entry-id">{editingEntry.id}</div>}
      <button
...
describe('ETBOverview Komponente', { timeout: 10000 }, () => {
  ...
  const mockFahrzeuge = {
    fahrzeugeImEinsatz: [{ kennzeichen: 'TEST-123', typ: 'ELW', funkrufname: 'Florian 1' }],
  };
