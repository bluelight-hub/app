    ...
    case 'USER':
      return (
        <Tooltip title="Meldung">
          <PiUser size={28} className="text-primary-500" />
        </Tooltip>
      );
    case 'LAGEMELDUNG':
      return (
        <Tooltip title="Lagemeldung">
          <PiPictureInPicture size={28} className="text-red-500" />
        </Tooltip>
      );
    case 'RESSOURCEN':
      return (
        <Tooltip title="Ressourcen">
          <PiAmbulance size={28} className="text-primary-500" />
        </Tooltip>
      );
    case 'BETROFFENE_PATIENTEN':
      return (
        <Tooltip title="Betroffene">
          <PiPlus size={28} className="text-primary-500" />
        </Tooltip>
      );
    case 'KORREKTUR':
      return (
        <Tooltip title="Korrektur">
          <PiArrowClockwiseBold size={28} className="text-orange-500" />
        </Tooltip>
      );
    default:
      return (
        <Tooltip title={category}>
          <PiMagnifyingGlass size={28} className="text-gray-500" />
        </Tooltip>
      );
  }
};
...
const ETBDashboardPage: React.FC = () => {
  ...
  const statistics = useMemo(() => {
    ...
    const lastEntryTime = items[0]?.timestampEreignis ? formatNatoDateTime(items[0].timestampEreignis) : '-';

    // Kategorien zählen
    const categoryCounts = items.reduce(
      (acc: Record<string, number>, item: EtbEntryDto) => {
        ...
        >
              ...
              <div className="flex items-center justify-between">
                ...
                <div>
                  {isUeberschrieben(entry) ? (
                    <Badge status="warning" text={<span className="text-base">Überschrieben</span>} className="ml-2" />
                  ) : (
                    <Badge status="success" text={<span className="text-base">Aktiv</span>} className="ml-2" />
                  )}
                </div>
              </div>
            }
          ...
          >
            <div className="flex flex-wrap gap-y-2 items-center">
              <Text strong className="mr-2 text-lg">
                #{entry.laufendeNummer}
              </Text>
              <Text className="text-lg">
                Von: <span className="font-medium">{entry.autorName}</span>
              </Text>
              {entry.abgeschlossenVon && (
                <Text className="ml-4 text-lg">
                  An: <span className="font-medium">{entry.abgeschlossenVon}</span>
                </Text>
              )}
            </div>
        ...
        <Row gutter={[16, 16]}>
          {statistics.totalEntries === 0 ? (
            ...
            <List className="w-full">
              <AnimatePresence initial={false}>{statistics.latestEntries.map(renderEntryCard)}</AnimatePresence>
            </List>
          )}
        </Row>
      </div>
