...
vi.mock('./DesktopSidebar', () => ({
  __esModule: true,
  default: vi.fn(({ navigation }) => <div data-testid="desktop-sidebar">{navigation.length} items</div>),
}));
...
describe('MainSidebar', () => {
  ...
  it('should render desktop sidebar by default', () => {
    render(<MainSidebar navigation={mockNavigation} />);
  ...
  it('should render mobile sidebar when isMobile is true and isOpen is true', () => {
    render(<MainSidebar isOpen={true} isMobile={true} navigation={mockNavigation} />);
  ...
  it('should not render mobile sidebar when isMobile is true but isOpen is false', () => {
    render(<MainSidebar isOpen={false} isMobile={true} navigation={mockNavigation} />);
  ...
  it('should have correct defaults for optional props', () => {
    const { rerender } = render(<MainSidebar navigation={mockNavigation} />);

    // Default: Only desktop sidebar is rendered
    expect(screen.queryByTestId('mobile-sidebar')).not.toBeInTheDocument();

    // When only isMobile=true but isOpen remains default (false)
    rerender(<MainSidebar isMobile={true} navigation={mockNavigation} />);
  ...
  it('should not render mobile sidebar when isMobile is false, even if isOpen is true', () => {
    render(<MainSidebar isOpen={true} isMobile={false} navigation={mockNavigation} />);
  ...
  it('should match snapshot', () => {
    const { container } = render(<MainSidebar navigation={mockNavigation} />);
