...
const InputWrapper: React.FC<InputWrapperProps> = ({ label, name, rules, children }) => (
  <Form.Item label={label} name={name} rules={rules} className="mb-4">
    {children}
  </Form.Item>
);
...
function FormLayout<T extends object>({ form, buttons, children }: FormLayoutProps<T>) {
  const [antForm] = Form.useForm();

  return (
    <Form form={antForm} initialValues={form.initialValues} onFinish={form.onFinish} layout="vertical">
      {children}
      <div className="mt-5">
...
export const ETBEntryForm: React.FC<ETBEntryFormProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <Input.TextArea rows={5} placeholder="Geben Sie hier den Inhalt des Einsatztagebucheintrags ein" />
      </InputWrapper>
