...
export const ETBTable: React.FC<ETBTableProps> = ({
   ...
   * Callback für das Überschreiben eines Eintrags
   */
  const ueberschreibeEntry = useCallback(
    (entry: EtbEntryDto) => {
  ...
  const columns = useMemo<TableColumnsType<EtbEntryDto>>(() => {
    ...
    return [
            ...
            case 'KORREKTUR':
              return (
                <Tooltip title="Korrektur">
                  <PiArrowClockwiseBold size={24} className="text-orange-500" />
                </Tooltip>
              );
            default:
              return value;
      ...
      },
      {
        ...
        render: (_, record) =>
          isUeberschrieben(record) ? <Tag color="warning">Überschrieben</Tag> : <Tag color="success">Aktiv</Tag>,
      },
      {
        title: 'Zeitpunkt',
