...
describe('ETBTable Komponente', { timeout: 10000 }, () => {
  ...
  it('sollte überschriebene Einträge visuell kennzeichnen', () => {
    ...
    const aktiverEintragElement = Array.from(beschreibungZellen).find((cell) =>
      cell.textContent?.includes('Aktiver Eintrag'),
    );

    const ueberschriebenerEintragElement = Array.from(beschreibungZellen).find((cell) =>
      cell.textContent?.includes('Überschriebener Eintrag'),
    );
  ...
  it('sollte clientseitige Filterung nach Status unterstützen', () => {
    ...
    const statusHeaderCell = Array.from(document.querySelectorAll('.ant-table-column-title')).find(
