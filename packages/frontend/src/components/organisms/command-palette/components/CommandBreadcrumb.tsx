...
export function CommandBreadcrumb({ commandStack, onBack, onNavigateTo }: CommandBreadcrumbProps) {
  ...
  return (
    <div className="flex items-center border-gray-200 border-b bg-gray-50/50 px-4 py-2.5 dark:border-gray-700 dark:bg-gray-800/30">
      <Button
        appearance="ghost"
        size="sm"
        onClick={onBack}
      ...
      <div className="flex items-center">
        {commandStack.map((cmd, idx) => {
          ...
          return (
            <Fragment key={cmd.id}>
              {idx > 0 && <PiCaretRight className="mx-1 h-3.5 w-3.5 text-gray-400 dark:text-gray-500" />}
              <button
                type="button"
