import { Button, Card, Form, Input, InputNumber, message, Modal, Select, Statistic, Table, Tabs, Tag } from 'antd';
...
const initialSachen: Sachschaden[] = [
  {
    id: 'S-101',
    bezeichnung: 'Fahrzeugschaden',
    beschreibung: 'PKW Frontalschaden',
    schaetzwert: 3000,
    status: 'in Bearbeitung',
  },
  {
    id: 'S-102',
    bezeichnung: 'Gebäudeschaden',
    beschreibung: 'Eingestürztes Garagendach',
    schaetzwert: 1500,
    status: 'gemeldet',
  },
];
...
const SchadensManagement: React.FC = () => {
  ...
  const handleSavePerson = (values: Personenschaden) => {
    if (editingPerson) {
      // Update
      setPersonen((prev) => prev.map((p) => (p.id === editingPerson.id ? { ...p, ...values } : p)));
  ...
  const handleSaveSach = (values: Sachschaden) => {
    if (editingSach) {
      // Update
      setSachen((prev) => prev.map((s) => (s.id === editingSach.id ? { ...s, ...values } : s)));
  ...
  return (
    <Card title="Schadensmanagement">
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  <div className="space-y-3">
                    <Statistic title="Leicht verletzt" value={countLeicht} valueStyle={{ color: '#52c41a' }} />
                    <Statistic title="Schwer verletzt" value={countSchwer} valueStyle={{ color: '#faad14' }} />
                    <Statistic title="Kritisch" value={countKritisch} valueStyle={{ color: '#f5222d' }} />
                  </div>
                </Card>
                ...
                >
                  <div className="space-y-3">
                    <Statistic title="Gemeldet" value={countGemeldet} valueStyle={{ color: '#1890ff' }} />
                    <Statistic title="In Bearbeitung" value={countInBearbeitung} valueStyle={{ color: '#722ed1' }} />
                    <Statistic title="Abgeschlossen" value={countAbgeschlossen} valueStyle={{ color: '#52c41a' }} />
                  </div>
                </Card>
              </div>
            ),
          },
          {
            ...
            children: (
              <>
                ...
                <Table dataSource={personen} columns={columnsPerson} rowKey="id" pagination={{ pageSize: 5 }} />
              </>
            ),
          },
          {
            ...
            children: (
              <>
                ...
                <Table dataSource={sachen} columns={columnsSach} rowKey="id" pagination={{ pageSize: 5 }} />
              </>
            ),
          },
        ]}
      ...
      >
        <Form form={formPerson} layout="vertical" onFinish={handleSavePerson}>
      ...
      <Modal
        ...
        cancelText="Abbrechen"
      >
        <Form form={formSach} layout="vertical" onFinish={handleSaveSach}>
          <Form.Item
            label="Bezeichnung"
            name="bezeichnung"
          ...
          >
            <InputNumber min={0} step={100} style={{ width: '100%' }} placeholder="z.B. 2000" />
          </Form.Item>
          <Form.Item label="Status" name="status" rules={[{ required: true, message: 'Bitte einen Status wählen' }]}>
