 ...
 */
const mockPersonalList: string[] = ['Max Mustermann', 'Julia Beispiel', 'Lukas Maier', 'Anna Müller'];

const mockVehicleList: string[] = ['RTW 1', 'RTW 2', 'NEF 1', 'MTF 3', 'KTW 4'];
...
const Einsatzabschnitte: React.FC = () => {
  ...
  return (
    <Card title="Einsatzabschnitte">
      ...
      <Tree treeData={treeData} defaultExpandAll onSelect={handleSelect} titleRender={titleRender} />

      {/* Info über ausgewählten Knoten */}
      {selectedNode && (
        <div className="mt-4 p-2 border rounded">
          <h4 className="font-semibold mb-1">Ausgewählter Abschnitt: {selectedNode.title}</h4>
          <p className="text-sm mb-1">{selectedNode.beschreibung || 'Keine Beschreibung'}</p>
          <p className="text-sm mb-1">
            <strong>Personal:</strong>{' '}
            {selectedNode.personnel?.length ? selectedNode.personnel.join(', ') : 'Keine Personen'}
          </p>
          <p className="text-sm mb-1">
            <strong>Fahrzeuge:</strong>{' '}
            {selectedNode.vehicles?.length ? selectedNode.vehicles.join(', ') : 'Keine Fahrzeuge'}
          </p>
        </div>
      )}
 ...
 */
function deleteNode(treeData: EinsatzabschnittNode[], keyToDelete: string): EinsatzabschnittNode[] {
  return treeData
    .map((node) => {
      if (node.children) {
        return {
