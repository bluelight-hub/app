    ...
    const columns: ColumnsType<Vehicle> = [
        ...
        },
        {
            ...
            render: (fms: number | undefined) =>
                fms !== undefined ? (
                    <Tooltip
                        title={`FMS-Status ${fms} (1=Einsatzklar, 2=Fahrt, 3=E-Stelle...)`}
                    >
                        {fms}
                    </Tooltip>
                ) : (
                ...
                <Form form={form} layout="vertical" onFinish={handleSave}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <Form.Item
                            label="FMS-Status"
                            name="fms"
                            tooltip="1=Einsatzklar, 2=Fahrt, 3=E-Stelle, 6=Werkstatt..."
                        >
    ...
    const handleMassAction = () => {
        ...
        const updated = vehicles.map((v) =>
            selectedRowKeys.includes(v.id)
                ? { ...v, status: 'Einsatzbereit' as VehicleStatus }
                : v
        );
        setVehicles(updated);
        message.success(
            `${selectedRowKeys.length} Fahrzeuge auf "Einsatzbereit" gesetzt ✅`
        );
