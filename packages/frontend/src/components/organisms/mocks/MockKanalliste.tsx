...
const Kanalliste: React.FC = () => {
  ...
  const handleSave = (values: FunkKanal) => {
    if (editingChannel) {
      ...
      setChannels((prev) => prev.map((c) => (c.id === editingChannel.id ? { ...values, id: editingChannel.id } : c)));
  ...
  return (
    ...
    >
      <Table dataSource={channels} columns={columns} rowKey="id" pagination={{ pageSize: 5 }} />
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSave}>
          <Form.Item label="Kanal" name="name" rules={[{ required: true, message: 'Bitte Kanalnamen eingeben' }]}>
