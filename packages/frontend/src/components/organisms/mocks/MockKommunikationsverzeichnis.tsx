...
const Kommunikationsverzeichnis: React.FC = () => {
  ...
  const handleSave = (values: Kontakt) => {
    if (editingKontakt) {
      // Update
      setKontakte((prev) => prev.map((k) => (k.id === editingKontakt.id ? { ...values, id: editingKontakt.id } : k)));
  ...
  return (
    ...
    >
      <Table dataSource={kontakte} columns={columns} rowKey="id" pagination={{ pageSize: 5 }} />
