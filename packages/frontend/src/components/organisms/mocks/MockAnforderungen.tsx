import { Button, Card, Form, Input, message, Modal, Table, Tabs, Timeline } from 'antd';
    ...
    <Card title="Anforderungsmanagement">
            ...
            children: (
              <>
                <div className="flex justify-end mb-4">
                  <Button type="primary" icon={<PiPlus />} onClick={handleNewAnforderung}>
                    Neue Anforderung
                  </Button>
                </div>
                <Table dataSource={anforderungen} columns={columns} rowKey="id" pagination={{ pageSize: 5 }} />
              </>
            ),
          },
          {
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSaveNew}>
          <Form.Item label="Titel" name="titel" rules={[{ required: true, message: 'Bitte einen Titel angeben' }]}>
            <Input />
          </Form.Item>
          <Form.Item
            label="Beschreibung"
