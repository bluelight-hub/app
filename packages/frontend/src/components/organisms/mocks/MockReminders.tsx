...
const WeckerUndErinnerungen = () => {
  ...
  const columns: ColumnType<Reminder>[] = [
    {
      ...
      render: (text: string, record: Reminder) => (
        <span className={record.done ? 'line-through text-gray-400' : ''}>{text}</span>
      ),
    },
    {
      title: 'Fällig um',
    ...
    },
    {
      ...
      render: (done: boolean) => <Tag color={done ? 'default' : 'processing'}>{done ? 'Erledigt' : 'Aktiv'}</Tag>,
    },
    {
      title: 'Wdh.',
      dataIndex: 'repeat',
    ...
    },
    {
      ...
      render: (_: unknown, record: Reminder) => (
        <div className="flex gap-2">
          {!record.done && (
            <Button type="primary" size="small" onClick={() => handleMarkDone(record.id)}>
              Erledigt
            </Button>
          )}
          <Button danger size="small" onClick={() => handleDeleteReminder(record.id)}>
            Löschen
          </Button>
  ...
  const handleSave = (values: FormValues) => {
    ...
    const newItem: Reminder = {
      ...
      repeat:
        values.repeatActive && values.intervalType && values.intervalValue && values.endAfter
          ? {
        ...
        >
          ...
          {timeMode === 'relative' && (
            <>
              ...
              <Form.Item label="Einheit (Minuten oder Stunden)" name="relativeIntervalType">
          ...
          {/* Wiederhol-Option */}
          <Form.Item label="Wiederholung aktiv?" name="repeatActive" valuePropName="checked">
            <Radio.Group>
