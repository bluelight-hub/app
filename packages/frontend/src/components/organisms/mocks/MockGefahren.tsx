import { Button, Card, Form, Input, message, Modal, Radio, Select, Table, Tag } from 'antd';
...
const Gefahren: React.FC = () => {
  ...
  const handleSave = (values: Gefahr) => {
    if (editingItem) {
      // Update
      setGefahren((prev) => prev.map((g) => (g.id === editingItem.id ? { ...g, ...values } : g)));
  ...
  return (
    <Card title="Gefahrenmanagement">
      ...
      <p className="mb-4 text-sm text-gray-500">Insgesamt {total} Gefahr(en) erfasst.</p>
      ...
      <Table dataSource={gefahren} columns={columns} rowKey="id" pagination={{ pageSize: 5 }} />
