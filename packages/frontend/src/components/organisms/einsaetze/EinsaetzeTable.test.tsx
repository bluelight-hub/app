...
vi.mock('antd', async () => {
  ...
  return {
    ...
    Table: ({ dataSource, columns, loading, onChange, rowSelection, pagination, locale }: any) => {
        ...
        <div data-testid="mock-table" data-loading={loading}>
          ...
          {pagination && (
            <div data-testid="pagination">
              <button onClick={() => onChange?.({ ...pagination, current: (pagination.current || 1) + 1 }, {}, {})}>
                Next Page
              </button>
            </div>
          )}
          <button data-testid="trigger-filter" onClick={() => onChange?.(pagination || {}, { name: ['test'] }, {})}>
            Apply Filter
          </button>
        </div>
      );
...
describe('EinsaetzeTable', () => {
  ...
  describe('Pagination', () => {
    ...
    it('sollte Pagination korrekt anzeigen', () => {
      render(
        <EinsaetzeTable einsaetze={mockEinsaetze} pagination={paginationConfig} onPageChange={mockOnPageChange} />,
      );
  ...
  describe('Sorting', () => {
    it('sollte Sortierung für Name-Spalte auslösen', () => {
      render(<EinsaetzeTable einsaetze={mockEinsaetze} onSortChange={mockOnSortChange} />);
  ...
  describe('Row Selection', () => {
    it('sollte Row Selection anzeigen, wenn onSelectionChange bereitgestellt wird', () => {
      render(<EinsaetzeTable einsaetze={mockEinsaetze} onSelectionChange={mockOnSelectionChange} />);
    ...
    it('sollte Zeilen-Auswahl korrekt behandeln', () => {
      ...
      expect(mockOnSelectionChange).toHaveBeenCalledWith(['einsatz-1'], [mockEinsaetze[0]]);
    });
    ...
    it('sollte selectedRowKeys korrekt verarbeiten', () => {
      ...
      expect(mockOnSelectionChange).toHaveBeenCalledWith(['einsatz-2'], [mockEinsaetze[1]]);
