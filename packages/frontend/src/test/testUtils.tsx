...
const AllProvidersWrapper = ({ children, queryClient, initialEntries = ['/'] }: AllProvidersWrapperProps) => {
  const client = queryClient || createTestQueryClient();
  return (
 ...
 */
export const renderWithQuery = (ui: ReactElement, options: CustomRenderOptions = {}) => {
  const { queryClient, ...renderOptions } = options;

  return render(ui, {
    wrapper: ({ children }) => <ReactQueryWrapper queryClient={queryClient}>{children}</ReactQueryWrapper>,
    ...renderOptions,
  });
};
...
export const renderWithProviders = (ui: ReactElement, options: CustomRenderOptions = {}) => {
  ...
  return render(ui, {
    wrapper: ({ children }) => (
      <AllProvidersWrapper queryClient={queryClient} initialEntries={initialEntries}>
        {children}
      </AllProvidersWrapper>
    ),
    ...renderOptions,
