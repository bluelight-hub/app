/**
 * Panel zum Zuweisen und Bearbeiten des taktischen Zeichens eines Fahrzeugs.
 *
 ...
 * Kräfte-Panels (PersonZuweisungPanel, FahrzeugZuweisungPanel, EinheitZeichenPanel).
 */
...
import { useFahrzeugZeichen } from '@/features/kraefte/api/use-fahrzeug-zeichen';
import { useUpdateFahrzeugZeichen } from '@/features/kraefte/api/use-update-fahrzeug-zeichen';

interface FahrzeugZeichenPanelProps {
  ...
  /** Fahrzeug-ID */
  fahrzeugId: string;
  /** Funkrufname des Fahrzeugs (für den Panel-Titel) */
  fahrzeugName: string;
}

/**
 * Slide-In Panel für die Zuweisung und Bearbeitung des taktischen Zeichens eines Fahrzeugs.
 *
 * Zeigt drei Bereiche:
 ...
 */
export function FahrzeugZeichenPanel({ isOpen, onClose, einsatzId, fahrzeugId, fahrzeugName }: FahrzeugZeichenPanelProps) {
  const queryClient = useQueryClient();
  const { data: zeichen, isLoading } = useFahrzeugZeichen(einsatzId, fahrzeugId);
  const { mutate: updateZeichen, isPending: isUpdating } = useUpdateFahrzeugZeichen(einsatzId, fahrzeugId);
  ...
  return (
    <Dialog.SlideIn
      isOpen={isOpen}
      onClose={onClose}
      title={`Taktisches Zeichen \u2014 ${fahrzeugName}`}
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          {!zeichen && (
            <section>
              <div className="rounded-panel border border-border-subtle bg-surface-raised px-4 py-6 text-center">
                ...
                <p className="mt-1 text-xs text-text-muted">Erstelle unten ein neues Zeichen für dieses Fahrzeug.</p>
              </div>
            </section>
          )}
          ...
          <section>
            ...
            <ZeichenEditor
              initialDefinition={aktuelleDefinition ?? { grundzeichen: 'kraftfahrzeug-landgebunden' }}
              initialLabel={zeichen?.label ?? fahrzeugName}
              onSave={handleSave}
              isSaving={isPending}
              onCancel={onClose}
            />
