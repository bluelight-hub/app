...
export function EtbTextCell({ entry, isDeleted, einsatzId }: EtbTextCellProps) {
  ...
  return (
    <div className="space-y-2">
      {/* ETB-Eintrag Text */}
      <p className={cn('whitespace-pre-wrap break-words text-sm leading-relaxed text-text-primary', isDeleted && 'text-text-muted line-through')}>{entry.text}</p>
      ...
      {linkedErinnerung && (
        <button
          ...
          className="mt-1 inline-flex items-center gap-1.5 rounded-pill bg-status-warning-surface px-2.5 py-1 font-medium text-status-warning-text text-xs transition-colors hover:bg-status-warning-surface/80 focus:outline-none focus-visible:shadow-focus-ring"
        >
      ...
      {linkedBefehlId && einsatzId && (
        <Link
          ...
          className="mt-1 inline-flex items-center gap-1.5 rounded-pill bg-action-secondary px-2.5 py-1 font-medium text-action-primary text-xs transition-colors hover:bg-action-secondary-hover focus:outline-none focus-visible:shadow-focus-ring"
        >
