  ...
  return (
    <Listbox as="div" value={selectedValue} onChange={handleChange} disabled={disabled}>
      <div className={cn('relative', className)}>
        ...
        >
          <PiTag className={cn('h-4 w-4 flex-shrink-0', isFilterActive ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400')} aria-hidden="true" />
        ...
        >
          ...
          {BASE_OPTIONS.map((option) => (
            ...
            >
              {({ selected }) => (
                <>
                  {option.value === 'untagged' ? <PiTagSimple className="h-4 w-4 text-gray-400" aria-hidden="true" /> : <PiTag className="h-4 w-4 text-gray-400" aria-hidden="true" />}
          ...
          {kategorieOptions.length > 0 && <div className="my-1 border-gray-200 border-t dark:border-gray-700" />}
