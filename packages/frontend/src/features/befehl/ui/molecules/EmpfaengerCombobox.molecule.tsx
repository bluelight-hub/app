 ...
 * Unterstuetzt Multi-Select mit Chips, manuelle Eingabe und Error-Fallback.
 */
...
export function EmpfaengerCombobox({ einsatzId, value, onChange, error }: EmpfaengerComboboxProps) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'Enter' && query.trim() && !isFetching && (!results || results.length === 0)) {
      e.preventDefault();
      addEmpfaenger({ name: query.trim() });
    }
  };

  /** Hinweistext unter dem Input */
  const getHintMessage = (): string | null => {
    if (isError) return 'Manuelle Eingabe möglich';
    if (isFetching && debouncedQuery.length >= 2) return 'Suche läuft...';
    if (query.length > 0 && query.length < 2) return 'Mind. 2 Zeichen eingeben';
    if (debouncedQuery.length >= 2 && results && results.length > 0 && filteredResults.length === 0) return 'Alle Treffer bereits ausgewählt';
    if (debouncedQuery.length >= 2 && results && results.length === 0) return 'Keine Treffer – Name manuell eingeben';
    return null;
  };
  ...
  return (
    <div>
      ...
      >
        <div className="relative">
          <ComboboxInput
            ...
            aria-describedby={hintMessage ? 'empfaenger-hint' : undefined}
            className={cn(
      ...
      {hintMessage && (
        <p id="empfaenger-hint" className="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
          {hintMessage}
        </p>
      )}
    </div>
