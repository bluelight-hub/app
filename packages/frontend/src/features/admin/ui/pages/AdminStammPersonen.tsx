...
import { ArchiveStammPersonDialog } from '../organisms/ArchiveStammPersonDialog';

/**
 * Admin Stamm-Personen Page.
 *
 * Zeigt Tabelle aller Stamm-Personen mit CRUD-Operationen.
 * Nur für authentifizierte Admins zugänglich.
 *
 * Keyboard Shortcuts:
 * - Ctrl+N / Cmd+N: Neue Person erstellen
 */
export function AdminStammPersonen() {
  ...
  const [editTarget, setEditTarget] = useState<StammPersonDto | null>(null);
  const [archiveTarget, setArchiveTarget] = useState<StammPersonDto | null>(null);
  ...
  // Handlers
  const handleCreatePerson = (data: CreateStammPersonDto) => {
    createStammPerson(data, {
      onSuccess: () => {
  ...
  const handleEditPerson = (person: StammPersonDto) => {
    setEditTarget(person);
    setIsEditDialogOpen(true);
  };

  const handleUpdatePerson = (data: UpdateStammPersonDto) => {
  ...
  const handleArchivePerson = (person: StammPersonDto) => {
    setArchiveTarget(person);
  };

  const confirmArchive = () => {
    if (!archiveTarget) return;
    archiveStammPerson(archiveTarget.id, {
      onSuccess: () => {
  ...
  const handleRestorePerson = (person: StammPersonDto) => {
  ...
  return (
    <ErrorBoundary>
      <Container maxWidth="6xl" className="py-8">
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <Text className="text-gray-600">Verwalten Sie die Mitglieder Ihrer Organisation.</Text>
            </div>
            <div className="flex items-center gap-3">
              <Button onClick={() => setShowArchived(!showArchived)} intent="secondary" appearance="ghost" size="sm">
                {showArchived ? <PiEyeSlash className="mr-2" /> : <PiEye className="mr-2" />}
        ...
        <ArchiveStammPersonDialog isOpen={!!archiveTarget} onClose={() => setArchiveTarget(null)} onConfirm={confirmArchive} person={archiveTarget} isArchiving={isArchiving} />
      </Container>
    </ErrorBoundary>
  );
}
