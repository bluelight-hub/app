...
import { RollenDefinitionenTable } from '../organisms/RollenDefinitionenTable';
import { CreateRollenDefinitionDialog } from '../organisms/CreateRollenDefinitionDialog';
import { EditRollenDefinitionDialog } from '../organisms/EditRollenDefinitionDialog';
import { DeactivateRollenDefinitionDialog } from '../organisms/DeactivateRollenDefinitionDialog';

/**
 * Admin Rollen-Definitionen Page.
 *
 * Zeigt Tabelle aller Rollendefinitionen mit CRUD-Operationen.
 * Nur fuer authentifizierte Admins zugaenglich.
 *
 * Keyboard Shortcuts:
 * - Ctrl+N / Cmd+N: Neue Rollendefinition erstellen
 */
export function AdminRollenDefinitionen() {
  ...
  const {
    ...
    error,
    refetch,
    createRollenDefinition,
    updateRollenDefinition,
    deactivateRollenDefinition,
    isCreating,
    isUpdating,
    isDeactivating,
    updatingId,
  ...
  const [editTarget, setEditTarget] = useState<RollenDefinitionDto | null>(null);
  const [deactivateTarget, setDeactivateTarget] = useState<RollenDefinitionDto | null>(null);
  ...
  // Handlers
  const handleCreateRollenDefinition = (data: CreateRollenDefinitionDto) => {
    createRollenDefinition(data, {
      onSuccess: () => {
  ...
  const handleEditRollenDefinition = (rollenDefinition: RollenDefinitionDto) => {
    setEditTarget(rollenDefinition);
    setIsEditDialogOpen(true);
  };

  const handleUpdateRollenDefinition = (data: UpdateRollenDefinitionDto) => {
  ...
  const handleDeactivateRollenDefinition = (rollenDefinition: RollenDefinitionDto) => {
    setDeactivateTarget(rollenDefinition);
  };

  const confirmDeactivate = () => {
    if (!deactivateTarget) return;
    deactivateRollenDefinition(deactivateTarget.id, {
      onSuccess: () => {
  ...
  return (
    <ErrorBoundary>
      <Container maxWidth="6xl" className="py-8">
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <Text className="text-gray-600">Verwalten Sie die verfuegbaren Rollendefinitionen fuer die taktische Uebersicht.</Text>
            </div>
        ...
        {/* Dialogs */}
        <CreateRollenDefinitionDialog isOpen={isCreateDialogOpen} onClose={() => setIsCreateDialogOpen(false)} onSubmit={handleCreateRollenDefinition} isSubmitting={isCreating} />

        <EditRollenDefinitionDialog
          isOpen={isEditDialogOpen}
          onClose={() => {
            setIsEditDialogOpen(false);
            setEditTarget(null);
          }}
          onSubmit={handleUpdateRollenDefinition}
          isSubmitting={isUpdating}
          rollenDefinition={editTarget}
        />

        <DeactivateRollenDefinitionDialog
          isOpen={!!deactivateTarget}
          onClose={() => setDeactivateTarget(null)}
          onConfirm={confirmDeactivate}
          rollenDefinition={deactivateTarget}
