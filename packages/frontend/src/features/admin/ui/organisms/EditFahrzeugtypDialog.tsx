...
const extractSollbesatzung = (value: FahrzeugtypDto['sollbesatzung']): SollbesatzungFormData => {
  ...
  return {
    fahrer: toNumber(record.fahrer),
    sanitaeter: toNumber(record.sanitaeter),
    notarzt: toNumber(record.notarzt),
    funktrupp: toNumber(record.funktrupp),
    helfer: toNumber(record.helfer),
  };
};
...
export const EditFahrzeugtypDialog = ({ isOpen, onClose, onSubmit, isSubmitting, fahrzeugtyp }: EditFahrzeugtypDialogProps) => {
  ...
  const form = useForm({
    ...
    validators: {
      onBlur: updateFahrzeugtypSchema,
      onSubmit: updateFahrzeugtypSchema,
    },
    onSubmit: ({ value }) => {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: form.reset ist stabil und lastLoadedIdRef verhindert unnötige Resets
  useEffect(() => {
    if (fahrzeugtyp && isOpen && lastLoadedIdRef.current !== fahrzeugtyp.id) {
      const sollbesatzung = extractSollbesatzung(fahrzeugtyp.sollbesatzung);

      lastLoadedIdRef.current = fahrzeugtyp.id;
      form.reset({
        code: fahrzeugtyp.code,
        bezeichnung: fahrzeugtyp.bezeichnung,
        kategorie: fahrzeugtyp.kategorie,
        beschreibung: fahrzeugtyp.beschreibung || '',
        istAktiv: fahrzeugtyp.istAktiv,
        fahrer: toNumberOrEmpty(sollbesatzung.fahrer),
        sanitaeter: toNumberOrEmpty(sollbesatzung.sanitaeter),
        notarzt: toNumberOrEmpty(sollbesatzung.notarzt),
        funktrupp: toNumberOrEmpty(sollbesatzung.funktrupp),
        helfer: toNumberOrEmpty(sollbesatzung.helfer),
      });

      setShowAdvanced(hasSollbesatzungValues(sollbesatzung));
    }
  }, [fahrzeugtyp, isOpen]);
  ...
  return (
    <Dialog isOpen={isOpen} onClose={handleClose}>
      <Dialog.Title>Fahrzeugtyp bearbeiten</Dialog.Title>
        ...
        <Dialog.Body>
          <div className="space-y-4">
            <form.Field name="code">
              {(field) => (
                <FormField label="Code" error={field.state.meta.errors[0]} required htmlFor="edit-fahrzeugtyp-code">
                  <Input
                    id="edit-fahrzeugtyp-code"
                    name={field.name}
            ...
            <form.Field name="bezeichnung">
              {(field) => (
                <FormField label="Bezeichnung" error={field.state.meta.errors[0]} required htmlFor="edit-fahrzeugtyp-bezeichnung">
                  <Input
                    id="edit-fahrzeugtyp-bezeichnung"
                    name={field.name}
                    value={field.state.value}
                    onBlur={field.handleBlur}
            ...
            <form.Field name="kategorie">
              {(field) => (
                <FormField label="Kategorie" required htmlFor="edit-fahrzeugtyp-kategorie">
                  <Select
                    id="edit-fahrzeugtyp-kategorie"
                    value={field.state.value}
                    onChange={(e) => field.handleChange(e.target.value as FahrzeugtypDto['kategorie'])}
            ...
            <form.Field name="beschreibung">
              {(field) => (
                <FormField label="Beschreibung" error={field.state.meta.errors[0]} htmlFor="edit-fahrzeugtyp-beschreibung">
                  <Textarea
                    id="edit-fahrzeugtyp-beschreibung"
                    name={field.name}
                    value={field.state.value}
                    onBlur={field.handleBlur}
                    onChange={(e) => field.handleChange(e.target.value)}
            ...
            <form.Field name="istAktiv">
              {(field) => (
                <FormField label="Status" htmlFor="edit-fahrzeugtyp-status">
                  <div className="flex items-center gap-3">
                    <Switch checked={field.state.value} onChange={field.handleChange} labelledBy="edit-fahrzeugtyp-status" />
                    <span id="edit-fahrzeugtyp-status" className="text-sm text-gray-700 dark:text-gray-300">
            ...
            <div className="rounded-lg border border-gray-200 p-3 dark:border-gray-700">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100">Sollbesatzung (optional)</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Definiert die empfohlene Besetzung für diesen Fahrzeugtyp.</p>
                </div>
              ...
              {showAdvanced && (
                <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <form.Field name="fahrer">
                    {(field) => (
                      <FormField label="Fahrer" error={field.state.meta.errors[0]} htmlFor="edit-fahrzeugtyp-fahrer">
                        <Input
                          id="edit-fahrzeugtyp-fahrer"
                          name={field.name}
                          type="number"
                          min={0}
                          max={99}
                  ...
                  <form.Field name="sanitaeter">
                    {(field) => (
                      <FormField label="Sanitäter" error={field.state.meta.errors[0]} htmlFor="edit-fahrzeugtyp-sanitaeter">
                        <Input
                          id="edit-fahrzeugtyp-sanitaeter"
                          name={field.name}
                          type="number"
                          min={0}
                          max={99}
                  ...
                  <form.Field name="notarzt">
                    {(field) => (
                      <FormField label="Notarzt" error={field.state.meta.errors[0]} htmlFor="edit-fahrzeugtyp-notarzt">
                        <Input
                          id="edit-fahrzeugtyp-notarzt"
                          name={field.name}
                          type="number"
                          min={0}
                          max={99}
                  ...
                  <form.Field name="funktrupp">
                    {(field) => (
                      <FormField label="Funktrupp" error={field.state.meta.errors[0]} htmlFor="edit-fahrzeugtyp-funktrupp">
                        <Input
                          id="edit-fahrzeugtyp-funktrupp"
                          name={field.name}
                          type="number"
                          min={0}
                          max={99}
                  ...
                  <form.Field name="helfer">
                    {(field) => (
                      <FormField label="Helfer" error={field.state.meta.errors[0]} htmlFor="edit-fahrzeugtyp-helfer">
                        <Input
                          id="edit-fahrzeugtyp-helfer"
                          name={field.name}
                          type="number"
                          min={0}
                          max={99}
