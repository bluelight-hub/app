...
import { Textarea } from '@/shared/ui/atoms/textarea.atom'; /** * Zod-Schema für CreateQualifikation Form. */
...
export const CreateQualifikationDialog = ({ isOpen, onClose, onSubmit, isSubmitting }: CreateQualifikationDialogProps) => {
  const form = useForm({
    defaultValues: { name: '', abkuerzung: '', kategorie: 'SONSTIGES' as QualifikationKategorie, beschreibung: '' },
    validatorAdapter: zodValidator(),
    validators: { onBlur: createQualifikationSchema },
    onSubmit: ({ value }) => {
      onSubmit({ name: value.name, abkuerzung: value.abkuerzung, kategorie: value.kategorie, beschreibung: value.beschreibung || undefined });
    },
  });
  const handleClose = () => {
    if (!isSubmitting) {
