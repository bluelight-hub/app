...
export const CreateUserDialog = ({ isOpen, onClose, onSubmit, isSubmitting }: CreateUserDialogProps) => {
  const form = useForm({
    defaultValues: { username: '', role: ManagedUserResponseDtoRoleEnum.User as ManagedUserResponseDtoRoleEnum },
    onSubmit: ({ value }) => {
      onSubmit(value);
    },
  });
      ...
      >
        ...
        <Dialog.Body>
          ...
          <div className="space-y-4">
            {' '}
            <form.Field name="username">{(field) => <UsernameField field={field} />}</form.Field> <form.Field name="role">{(field) => <RoleField field={field} />}</form.Field>{' '}
          </div>{' '}
        </Dialog.Body>{' '}
        <Dialog.Footer>
          {' '}
