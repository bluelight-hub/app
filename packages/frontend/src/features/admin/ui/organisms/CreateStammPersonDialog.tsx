...
import { z } from 'zod';
import type { CreateStammPersonDto, QualifikationDto } from '@bluelight-hub/shared/client';
...
/**
 * Zod-Schema für CreateStammPerson Form.
 */
...
export const CreateStammPersonDialog = ({ isOpen, onClose, onSubmit, isSubmitting, qualifikationen, qualifikationenLoading }: CreateStammPersonDialogProps) => {
  const form = useForm({
    ...
    validators: {
      onBlur: createStammPersonSchema,
    },
    onSubmit: ({ value }) => {
      onSubmit({
  ...
  return (
    <Dialog isOpen={isOpen} onClose={handleClose}>
      <Dialog.Title>Neue Person erstellen</Dialog.Title>
        ...
        <Dialog.Body>
          <div className="space-y-4">
            ...
            <form.Field name="vorname">
              {(field) => (
                <FormField label="Vorname" error={field.state.meta.errors[0]} required htmlFor="create-person-vorname">
                  <Input
                    id="create-person-vorname"
                    name={field.name}
            ...
            <form.Field name="nachname">
              {(field) => (
                <FormField label="Nachname" error={field.state.meta.errors[0]} required htmlFor="create-person-nachname">
                  <Input
                    id="create-person-nachname"
                    name={field.name}
                    value={field.state.value}
                    onBlur={field.handleBlur}
                    onChange={(e) => field.handleChange(e.target.value)}
            ...
            {/* Personalnummer */}
            <form.Field name="personalnummer">
              {(field) => (
                <FormField label="Personalnummer" error={field.state.meta.errors[0]} required htmlFor="create-person-personalnummer">
                  <Input
                    ...
                    onBlur={field.handleBlur}
                    onChange={(e) => field.handleChange(e.target.value)}
                    placeholder="z.B. 12345 oder MA-2024-001"
                    variant={field.state.meta.errors.length > 0 ? 'error' : 'default'}
                    fullWidth
                  />
                </FormField>
            ...
            <form.Field name="funkkenungBOS">
              {(field) => (
                <FormField label="BOS-Funkkennung" error={field.state.meta.errors[0]} htmlFor="create-person-funkkenungbos">
                  <Input
                    id="create-person-funkkenungbos"
                    name={field.name}
                    value={field.state.value}
                    onBlur={field.handleBlur}
                    onChange={(e) => field.handleChange(e.target.value)}
            ...
            <form.Field name="qualifikationIds">
              {(field) => (
                <FormField label="Qualifikationen" error={field.state.meta.errors[0]} htmlFor="create-person-qualifikationen">
                  {qualifikationenLoading ? (
                    ...
                    <div className="max-h-48 space-y-2 overflow-y-auto rounded-lg border border-gray-200 p-3 dark:border-gray-700">
                      {activeQualifikationen.map((qual) => (
                        <label key={qual.id} htmlFor={`create-person-qual-${qual.id}`} className="flex cursor-pointer items-center gap-2 rounded p-1 hover:bg-gray-50 dark:hover:bg-gray-800">
                          <Checkbox
                            id={`create-person-qual-${qual.id}`}
