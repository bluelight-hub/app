  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('sollbesatzung', {
        ...
        cell: ({ row }) => <span className="whitespace-nowrap text-gray-600 text-sm dark:text-gray-400">{formatSollbesatzungCompact(row.original.sollbesatzung)}</span>,
      }),
  ...
  if (fahrzeugtypen.length === 0) {
    return (
      <div className="flex h-48 flex-col items-center justify-center p-8">
        <Text className="text-gray-600">Keine Fahrzeugtypen vorhanden.</Text>
        <Text className="text-gray-500 text-sm">Erstellen Sie einen neuen Fahrzeugtyp.</Text>
      </div>
    );
  }
