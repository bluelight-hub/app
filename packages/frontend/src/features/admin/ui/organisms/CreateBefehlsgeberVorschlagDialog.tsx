...
import { Input } from '@/shared/ui/atoms/input.atom'; /** * Zod-Schema fuer CreateBefehlsgeberVorschlag Form. */
...
export const CreateBefehlsgeberVorschlagDialog = ({ isOpen, onClose, onSubmit, isSubmitting }: CreateBefehlsgeberVorschlagDialogProps) => {
  const form = useForm({
    defaultValues: { kuerzel: '', label: '', sortOrder: 0 },
    validatorAdapter: zodValidator(),
    validators: { onBlur: createBefehlsgeberVorschlagSchema },
    onSubmit: ({ value }) => {
      onSubmit({ kuerzel: value.kuerzel, label: value.label, sortOrder: value.sortOrder ?? 0 });
    },
  });
  const handleClose = () => {
    if (!isSubmitting) {
