...
interface DeleteBefehlsgeberVorschlagDialogProps {
  ...
  isDeleting: boolean;
} /** * Bestaetigungsdialog zum Loeschen eines Befehlsgeber-Vorschlags. * * Warnt den Benutzer, dass der Vorschlag unwiderruflich geloescht wird. */
    ...
    <Dialog isOpen={isOpen} onClose={onClose}>
      ...
      <Dialog.Title>
        ...
        <div className="flex items-center gap-2 text-status-danger-text">
          {' '}
          <PiWarning className="h-5 w-5" /> Befehlsgeber-Vorschlag löschen{' '}
        </div>{' '}
      </Dialog.Title>{' '}
      <Dialog.Body>
        {' '}
            ...
            <div className="flex items-start justify-between">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  {' '}
                  <span className="font-mono font-semibold text-text-primary">{vorschlag.kuerzel}</span>{' '}
                </div>{' '}
                <Text className="mt-1 text-text-secondary">{vorschlag.label}</Text>{' '}
              </div>{' '}
            </div>{' '}
          </div>{' '}
