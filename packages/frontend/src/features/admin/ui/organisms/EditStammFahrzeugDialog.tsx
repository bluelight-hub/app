...
import { z } from 'zod';
import type { StammFahrzeugDto, UpdateStammFahrzeugDto } from '@bluelight-hub/shared/client';
...
/**
 * Zod-Schema für UpdateStammFahrzeug Form.
 * Hinweis: fahrzeugtypId ist IMMUTABLE und kann nicht geändert werden.
 */
...
/**
 * Dialog zum Bearbeiten eines Stamm-Fahrzeugs.
 * Hinweis: Fahrzeugtyp kann nicht geändert werden (IMMUTABLE).
 */
export const EditStammFahrzeugDialog = ({ isOpen, onClose, onSubmit, isSubmitting, fahrzeug }: EditStammFahrzeugDialogProps) => {
  const form = useForm({
    ...
    validators: {
      onBlur: updateStammFahrzeugSchema,
    },
    onSubmit: ({ value }) => {
      onSubmit({
  ...
  // Sync form values when fahrzeug changes
  useEffect(() => {
    if (fahrzeug) {
      form.reset();
      form.setFieldValue('rufname', fahrzeug.rufname);
      form.setFieldValue('funkrufname', fahrzeug.funkrufname);
      form.setFieldValue('kennzeichen', fahrzeug.kennzeichen || '');
      form.setFieldValue('baujahr', fahrzeug.baujahr || '');
      form.setFieldValue('funkkenungBOS', fahrzeug.funkkenungBOS || '');
    }
  }, [fahrzeug, form]);
  ...
  return (
    <Dialog isOpen={isOpen} onClose={handleClose}>
      <Dialog.Title>Fahrzeug bearbeiten</Dialog.Title>
      ...
      >
        <Dialog.Body>
          <div className="space-y-4">
            {/* Fahrzeugtyp (IMMUTABLE - nur anzeigen) */}
            {fahrzeug && (
              <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800">
                <Text size="sm" color="muted" className="mb-1">
                  Fahrzeugtyp (nicht änderbar)
                </Text>
                <Badge variant="info">
                  {fahrzeug.fahrzeugtyp.code} - {fahrzeug.fahrzeugtyp.bezeichnung}
                </Badge>
              </div>
            )}
            ...
            <form.Field name="rufname">
              {(field) => (
                <FormField label="Rufname" error={field.state.meta.errors[0]} required htmlFor="edit-fahrzeug-rufname">
                  <Input
                    id="edit-fahrzeug-rufname"
                    name={field.name}
            ...
            <form.Field name="funkrufname">
              {(field) => (
                <FormField label="Funkrufname" error={field.state.meta.errors[0]} required htmlFor="edit-fahrzeug-funkrufname">
                  <Input
                    id="edit-fahrzeug-funkrufname"
                    name={field.name}
                    value={field.state.value}
                    onBlur={field.handleBlur}
                    onChange={(e) => field.handleChange(e.target.value)}
            ...
            <form.Field name="kennzeichen">
              {(field) => (
                <FormField label="Kennzeichen" error={field.state.meta.errors[0]} htmlFor="edit-fahrzeug-kennzeichen">
                  <Input
                    id="edit-fahrzeug-kennzeichen"
                    name={field.name}
                    value={field.state.value}
                    onBlur={field.handleBlur}
            ...
            <form.Field name="baujahr">
              {(field) => (
                <FormField label="Baujahr" error={field.state.meta.errors[0]} htmlFor="edit-fahrzeug-baujahr">
                  <Input
                    id="edit-fahrzeug-baujahr"
                    name={field.name}
                    type="number"
                    value={field.state.value}
                    onBlur={field.handleBlur}
            ...
            <form.Field name="funkkenungBOS">
              {(field) => (
                <FormField label="BOS-Funkkennung" error={field.state.meta.errors[0]} htmlFor="edit-fahrzeug-funkkenungbos">
                  <Input
                    id="edit-fahrzeug-funkkenungbos"
                    name={field.name}
                    value={field.state.value}
                    onBlur={field.handleBlur}
                    onChange={(e) => field.handleChange(e.target.value)}
