              ...
              {[1, 2, 3, 4, 5].map((rowId) => (
                <Table.Row key={`skeleton-row-${rowId}`}>
                  {[1, 2, 3, 4, 5, 6, 7].map((cellId) => (
                    <Table.Cell key={`skeleton-cell-${rowId}-${cellId}`}>
  ...
  if (!invites || invites.length === 0) {
    return (
      <div className="flex h-64 flex-col items-center justify-center rounded-lg border border-dashed border-gray-300 p-8 dark:border-gray-700">
        ...
        <p className="mb-1 text-lg font-medium text-gray-900 dark:text-gray-100">Keine Invite-Codes vorhanden</p>
        <p className="text-sm text-gray-500 dark:text-gray-400">Es wurden noch keine Invite-Codes erstellt.</p>
      </div>
    );
  }
