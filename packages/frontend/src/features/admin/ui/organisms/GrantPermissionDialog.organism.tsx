...
/**
 * Dialog zum Gewaehren einer neuen Custom Permission.
 *
 * Story 5.2 AC1: Domain-Select + Action-Select aus Available Permissions
 * mit Duplikat-Check (bereits vergebene Permissions ausgeschlossen).
 */
export function GrantPermissionDialog({ isOpen, onClose, onGrant, isGranting, existingPermissions, username }: GrantPermissionDialogProps) {
  ...
  const selectedAction = useStore(form.store, (state) => state.values.action);

  /** Verfuegbare Actions fuer gewaehlte Domain (ohne bereits vergebene) */
  ...
  const domainOptions = useMemo(() => (available ?? []).map((a) => ({ value: a.domain, label: `${a.domain} - ${a.description}` })), [available]);

  /** Action-Optionen fuer das Select */
        ...
        <Dialog.Body>
          {isLoading ? (
            <div className="flex items-center gap-2 py-4">
              <Spinner size="sm" />
              <span className="text-gray-500 text-sm">Verfuegbare Permissions laden...</span>
            </div>
          ) : (
                  ...
                  {(field) => (
                    <FormField label="Aktion" required error={field.state.meta.errors[0]} htmlFor="grant-perm-action">
                      <Select
                        ...
                        fullWidth
                      />
                      {availableActions.length === 0 && selectedDomain && <p className="mt-1 text-gray-500 text-sm">Alle Aktionen dieser Domain sind bereits vergeben.</p>}
                    </FormField>
                  )}
                </form.Field>
              )}
