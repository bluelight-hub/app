/**
 * Tests für `useEigenschutzPsaQuittungLive` (Story 3.4 AC11).
 *
 * Mocks `socket.io-client` mit einem Manual-Event-Emitter und prüft das
 * Connect-Lifecycle, Payload-Validation, Dedup und das Cache-Invalidate-
 * Verhalten (Story-3.4-spezifisch — keine Banner-Logik, nur Invalidator).
 */
...
import { useEigenschutzPsaQuittungLive } from '../use-eigenschutz-psa-quittung-live';
...
const validPayload = (overrides: Partial<Record<string, unknown>> = {}) => ({
  ...
  userIdHash: 'hash-abc',
  quittiertAm: '2026-04-24T10:30:00.000Z',
  occurredAt: '2026-04-24T10:30:00.000Z',
  ...overrides,
});

describe('useEigenschutzPsaQuittungLive (Story 3.4 AC11)', () => {
  beforeEach(() => {
    ioMock.mockReset();
  ...
  it('öffnet WS-Verbindung und joint den Einsatz-Room', () => {
    ...
    renderHook(() => useEigenschutzPsaQuittungLive({ einsatzId: 'einsatz-1' }), { wrapper: wrapper(client) });
  ...
  it('invalidiert Quittungen, offene Bekanntgaben und Ampelstatus bei validem Frame', () => {
    const client = makeClient();
    const invalidateSpy = vi.spyOn(client, 'invalidateQueries');
    renderHook(() => useEigenschutzPsaQuittungLive({ einsatzId: 'einsatz-1' }), { wrapper: wrapper(client) });

    const handler = getHandler('eigenschutz:psa-quittung-abgegeben');
