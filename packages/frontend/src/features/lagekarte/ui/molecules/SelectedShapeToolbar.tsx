...
export const SelectedShapeToolbar: React.FC<SelectedShapeToolbarProps> = ({ shapeId: _shapeId, position, onEdit, onDelete, onChangeStyle }) => {
  return (
    <div
      className="absolute z-50 flex gap-1 rounded-lg border border-gray-200 bg-white/90 p-1 shadow-lg backdrop-blur-sm"
      style={{
        left: `${position.x}px`,
        top: `${position.y}px`,
    ...
    >
      ...
      >
        <PiPencil className="h-4 w-4 text-gray-700" />
      </button>
      ...
      >
        <PiPalette className="h-4 w-4 text-gray-700" />
      </button>
    </div>
  );
};
