 ...
 */
export const useEinsatztagebuch = ({ filterParams = {}, refetchInterval }: UseEinsatztagebuchOptions = {}) => {
  ...
  const archiveEinsatztagebuchEintrag = useMutation({
    ...
    onMutate: async ({ nummer }) => {
      ...
      queryClient.setQueryData<EtbEntriesResponse>([...ETB_KEYS.entries(), params], (old) => {
        ...
        return {
          ...old,
          items: old.items.map((item) => (item.laufendeNummer === nummer ? { ...item, istAbgeschlossen: true } : item)),
          data: {
            ...old.pagination,
  ...
  const ueberschreibeEinsatztagebuchEintrag = useMutation({
    mutationFn: async ({ id, beschreibung }: { id: string; beschreibung: string }) => {
  ...
  const changePage = async (page: number, limit: number) => {
    ...
    try {
      ...
      const response = await api.etb.etbControllerFindAllV1(requestObj, {
        headers: {
          'Content-Type': 'application/json',
        },
      });
