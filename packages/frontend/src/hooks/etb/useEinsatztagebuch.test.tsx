...
describe('useEinsatztagebuch Hook', () => {
  ...
  it('sollte Fehler bei der Abfrage korrekt behandeln', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith('Fehler beim Abrufen der Einsatztagebuch-Daten:', testError);
  });
  ...
  it('sollte Fehler bei Mutation korrekt behandeln', async () => {
    ...
    // Prüfen, ob der Logger mit dem Fehler aufgerufen wurde
    await waitFor(() => {
      expect(logger.error).toHaveBeenCalledWith('Fehler beim Überschreiben des Eintrags:', testError);
    });
  });
