...
export class EinsatzApi extends runtime.BaseAPI {
  ...
  async einsatzControllerCreateV1Raw(
    ...
    return new runtime.JSONApiResponse(response, (jsonValue) => EinsatzFromJSON(jsonValue));
  }
  ...
  async einsatzControllerFindAllV1Raw(
    ...
    return new runtime.JSONApiResponse(response, (jsonValue) => jsonValue.map(EinsatzFromJSON));
  }
  ...
  async einsatzControllerFindByIdV1(
    ...
    const response = await this.einsatzControllerFindByIdV1Raw(requestParameters, initOverrides);
    return await response.value();
  }
}
