/* tslint:disable */
/* eslint-disable */
/**
 * BlueLight Hub API (Alpha)
 * BlueLight Hub API for the BlueLight Hub application.  ## Server-Access-Token (X-Server-Access-Token)  Alle API-Endpunkte (außer /health und /setup) erfordern einen gültigen Server-Access-Token im Header:  ``` X-Server-Access-Token: <plaintext_token> ```  **Multi-Token Support:** - Mehrere aktive Tokens gleichzeitig möglich - Jedes Token hat einen eindeutigen Namen zur Identifikation - `lastUsedAt` wird bei jeder erfolgreichen Validierung aktualisiert - Tokens können individuell deaktiviert/reaktiviert/rotiert werden  **Token-Namenskonventionen (Best Practices):** - `Desktop Hauptwache` - für Desktop-App der Hauptwache - `Mobile SEG Nord` - für Mobile App der SEG Nord - `Integration Server` - für automatisierte Systeme - Bei Rotation: Datum im Namen (z.B. \"Desktop HW 2026-01\")
 *
 * The version of the OpenAPI document: 1.0.0-alpha.103-alpha
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface AckPsaQuittungDto
 */
export interface AckPsaQuittungDto {
    /**
     * cuid2 der Einheit, die die Bekanntgabe quittiert.
     * @type {string}
     * @memberof AckPsaQuittungDto
     */
    einheitId: string;
}

/**
 * Check if a given object implements the AckPsaQuittungDto interface.
 */
export function instanceOfAckPsaQuittungDto(value: object): value is AckPsaQuittungDto {
    if (!('einheitId' in value) || value['einheitId'] === undefined) return false;
    return true;
}

export function AckPsaQuittungDtoFromJSON(json: any): AckPsaQuittungDto {
    return AckPsaQuittungDtoFromJSONTyped(json, false);
}

export function AckPsaQuittungDtoFromJSONTyped(json: any, ignoreDiscriminator: boolean): AckPsaQuittungDto {
    if (json == null) {
        return json;
    }
    return {
        
        'einheitId': json['einheitId'],
    };
}

export function AckPsaQuittungDtoToJSON(json: any): AckPsaQuittungDto {
    return AckPsaQuittungDtoToJSONTyped(json, false);
}

export function AckPsaQuittungDtoToJSONTyped(value?: AckPsaQuittungDto | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'einheitId': value['einheitId'],
    };
}

