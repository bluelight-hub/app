/* tslint:disable */
/* eslint-disable */
/**
 * BlueLight Hub API (Alpha)
 * BlueLight Hub API for the BlueLight Hub application.  ## Server-Access-Token (X-Server-Access-Token)  Alle API-Endpunkte (außer /health und /setup) erfordern einen gültigen Server-Access-Token im Header:  ``` X-Server-Access-Token: <plaintext_token> ```  **Multi-Token Support:** - Mehrere aktive Tokens gleichzeitig möglich - Jedes Token hat einen eindeutigen Namen zur Identifikation - `lastUsedAt` wird bei jeder erfolgreichen Validierung aktualisiert - Tokens können individuell deaktiviert/reaktiviert/rotiert werden  **Token-Namenskonventionen (Best Practices):** - `Desktop Hauptwache` - für Desktop-App der Hauptwache - `Mobile SEG Nord` - für Mobile App der SEG Nord - `Integration Server` - für automatisierte Systeme - Bei Rotation: Datum im Namen (z.B. \"Desktop HW 2026-01\")
 *
 * The version of the OpenAPI document: 1.0.0-alpha.102-alpha
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface ExterneUserDto
 */
export interface ExterneUserDto {
    /**
     * Eindeutige User-ID
     * @type {string}
     * @memberof ExterneUserDto
     */
    id: string;
    /**
     * Benutzername
     * @type {string}
     * @memberof ExterneUserDto
     */
    username: string;
}

/**
 * Check if a given object implements the ExterneUserDto interface.
 */
export function instanceOfExterneUserDto(value: object): value is ExterneUserDto {
    if (!('id' in value) || value['id'] === undefined) return false;
    if (!('username' in value) || value['username'] === undefined) return false;
    return true;
}

export function ExterneUserDtoFromJSON(json: any): ExterneUserDto {
    return ExterneUserDtoFromJSONTyped(json, false);
}

export function ExterneUserDtoFromJSONTyped(json: any, ignoreDiscriminator: boolean): ExterneUserDto {
    if (json == null) {
        return json;
    }
    return {
        
        'id': json['id'],
        'username': json['username'],
    };
}

export function ExterneUserDtoToJSON(json: any): ExterneUserDto {
    return ExterneUserDtoToJSONTyped(json, false);
}

export function ExterneUserDtoToJSONTyped(value?: ExterneUserDto | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'id': value['id'],
        'username': value['username'],
    };
}

