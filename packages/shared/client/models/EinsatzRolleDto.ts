/* tslint:disable */
/* eslint-disable */
/**
 * BlueLight Hub API (Alpha)
 * BlueLight Hub API for the BlueLight Hub application.  ## Server-Access-Token (X-Server-Access-Token)  Alle API-Endpunkte (außer /health und /setup) erfordern einen gültigen Server-Access-Token im Header:  ``` X-Server-Access-Token: <plaintext_token> ```  **Multi-Token Support:** - Mehrere aktive Tokens gleichzeitig möglich - Jedes Token hat einen eindeutigen Namen zur Identifikation - `lastUsedAt` wird bei jeder erfolgreichen Validierung aktualisiert - Tokens können individuell deaktiviert/reaktiviert/rotiert werden  **Token-Namenskonventionen (Best Practices):** - `Desktop Hauptwache` - für Desktop-App der Hauptwache - `Mobile SEG Nord` - für Mobile App der SEG Nord - `Integration Server` - für automatisierte Systeme - Bei Rotation: Datum im Namen (z.B. \"Desktop HW 2026-01\")
 *
 * The version of the OpenAPI document: 1.0.0-alpha.104-alpha
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface EinsatzRolleDto
 */
export interface EinsatzRolleDto {
    /**
     * User-ID (CUID2)
     * @type {string}
     * @memberof EinsatzRolleDto
     */
    userId: string;
    /**
     * Username des zugewiesenen Users
     * @type {string}
     * @memberof EinsatzRolleDto
     */
    userName: string;
    /**
     * Zugewiesene Rolle im Einsatz
     * @type {string}
     * @memberof EinsatzRolleDto
     */
    rolle: EinsatzRolleDtoRolleEnum;
}


/**
 * @export
 */
export const EinsatzRolleDtoRolleEnum = {
    Befehlsgeber: 'BEFEHLSGEBER',
    Ersteller: 'ERSTELLER',
    Empfaenger: 'EMPFAENGER',
    Beobachter: 'BEOBACHTER'
} as const;
export type EinsatzRolleDtoRolleEnum = typeof EinsatzRolleDtoRolleEnum[keyof typeof EinsatzRolleDtoRolleEnum];


/**
 * Check if a given object implements the EinsatzRolleDto interface.
 */
export function instanceOfEinsatzRolleDto(value: object): value is EinsatzRolleDto {
    if (!('userId' in value) || value['userId'] === undefined) return false;
    if (!('userName' in value) || value['userName'] === undefined) return false;
    if (!('rolle' in value) || value['rolle'] === undefined) return false;
    return true;
}

export function EinsatzRolleDtoFromJSON(json: any): EinsatzRolleDto {
    return EinsatzRolleDtoFromJSONTyped(json, false);
}

export function EinsatzRolleDtoFromJSONTyped(json: any, ignoreDiscriminator: boolean): EinsatzRolleDto {
    if (json == null) {
        return json;
    }
    return {
        
        'userId': json['userId'],
        'userName': json['userName'],
        'rolle': json['rolle'],
    };
}

export function EinsatzRolleDtoToJSON(json: any): EinsatzRolleDto {
    return EinsatzRolleDtoToJSONTyped(json, false);
}

export function EinsatzRolleDtoToJSONTyped(value?: EinsatzRolleDto | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'userId': value['userId'],
        'userName': value['userName'],
        'rolle': value['rolle'],
    };
}

