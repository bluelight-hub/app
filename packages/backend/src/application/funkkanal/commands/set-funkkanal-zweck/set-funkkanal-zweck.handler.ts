...
import type { SetFunkkanalZweckCommand } from './set-funkkanal-zweck.command';

/**
 * Handler zum Setzen des Zweck-Texts eines Funkkanals.
 */
@Injectable()
export class SetFunkkanalZweckHandler extends TransactionalCommandHandler<SetFunkkanalZweckCommand, FunkkanalAggregate> {
  constructor(
  ...
  protected async executeInTransaction(command: SetFunkkanalZweckCommand, tx: TransactionContext): Promise<Result<FunkkanalAggregate> | { result: FunkkanalAggregate; events: DomainEvent[] }> {
    ...
    const mutation = aggregate.setZweck(command.zweck, command.userId);
    if (mutation.isFailure) {
      return Result.fail<FunkkanalAggregate>(mutation.error ?? 'setZweck fehlgeschlagen');
    }
