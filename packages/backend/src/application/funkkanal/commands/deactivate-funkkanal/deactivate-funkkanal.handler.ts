...
import type { DeactivateFunkkanalCommand } from './deactivate-funkkanal.command';

/**
 * Handler zum Deaktivieren eines Funkkanals (Status `inaktiv`).
 */
@Injectable()
export class DeactivateFunkkanalHandler extends TransactionalCommandHandler<DeactivateFunkkanalCommand, FunkkanalAggregate> {
  constructor(
  ...
  protected async executeInTransaction(command: DeactivateFunkkanalCommand, tx: TransactionContext): Promise<Result<FunkkanalAggregate> | { result: FunkkanalAggregate; events: DomainEvent[] }> {
    ...
    const mutation = aggregate.deactivate(command.userId);
    if (mutation.isFailure) {
      return Result.fail<FunkkanalAggregate>(mutation.error ?? 'deactivate fehlgeschlagen');
    }
