...
import { SetFunkkanalSortIndexHandler } from '../set-funkkanal-sort-index.handler';
import { SetFunkkanalSortIndexCommand } from '../set-funkkanal-sort-index.command';
...
describe('SetFunkkanalSortIndexHandler', () => {
  let handler: SetFunkkanalSortIndexHandler;
  ...
  beforeEach(async () => {
    ...
    handler = module.get(SetFunkkanalSortIndexHandler);
  });

  it('setzt sortIndex und emittiert Geändert-Event', async () => {
    const a = make();
    a.clearDomainEvents();
    mockRepo.findById.mockResolvedValue(a);

    const cmd = SetFunkkanalSortIndexCommand.create({ kanalId: a.id.value, sortIndex: 5, userId: userId() }).value!;
    const result = await handler.execute(cmd);

    expect(result.isSuccess).toBe(true);
    expect(result.value!.sortIndex).toBe(5);
  ...
  it('meldet Fehler bei nicht existierendem Kanal', async () => {
    ...
    const cmd = SetFunkkanalSortIndexCommand.create({ kanalId: 'a'.repeat(24), sortIndex: 0, userId: userId() }).value!;
    const result = await handler.execute(cmd);
    expect(result.isFailure).toBe(true);
  });

  it('Command lehnt negative sortIndex ab', () => {
    const r = SetFunkkanalSortIndexCommand.create({ kanalId: 'a'.repeat(24), sortIndex: -1, userId: userId() });
