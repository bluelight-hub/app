...
export class ErfasseFahrzeugAusStammdatenHandler extends TransactionalCommandHandler<ErfasseFahrzeugAusStammdatenCommand, EinsatzFahrzeugDto> {
  ...
  protected async executeInTransaction(
    ...
    // 8. Default-Zeichen synchron in derselben Transaktion erstellen
    try {
      ...
      if (defaultsResult.isSuccess && defaultsResult.value) {
        const match = defaultsResult.value.find((d) => d.referenzId === stammFahrzeug.fahrzeugtypId);
        if (match) {
    ...
    // 9. Extract Domain Events (für Outbox - AC3)
