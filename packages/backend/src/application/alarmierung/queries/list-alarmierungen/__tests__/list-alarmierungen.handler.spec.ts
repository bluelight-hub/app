...
function makeAggregate(zeit: Date) {
...
describe('ListAlarmierungenQueryHandler', () => {
  ...
  beforeEach(async () => {
    mockRepo = { findByEinsatzId: jest.fn() };
  ...
  it('liefert Alarmierungen DESC nach alarmierungszeit', async () => {
    ...
    const query = ListAlarmierungenQuery.create({ einsatzId: EinsatzId.create().value!.value }).value!;
    const result = await handler.execute(query);

    expect(result.isSuccess).toBe(true);
    expect(result.value!.map((a) => a.alarmierungszeit.getTime())).toEqual([
      new Date('2026-04-15T11:00:00Z').getTime(),
      new Date('2026-04-15T10:00:00Z').getTime(),
      new Date('2026-04-15T09:00:00Z').getTime(),
    ]);
  });
