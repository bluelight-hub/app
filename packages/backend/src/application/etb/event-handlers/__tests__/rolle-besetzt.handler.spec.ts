...
import { RolleBesetztEventHandler } from '../rolle-besetzt.handler';
...
/**
 * Erstellt ein Test-RolleBesetzt Event mit allen erforderlichen Daten.
 *
...
function createTestEvent(
  ...
  return new RolleBesetzt(
    ...
    overrides.besetztVon ?? generateTestCuid(),
  );
}

describe('RolleBesetztEventHandler', () => {
  let handler: RolleBesetztEventHandler;
  ...
  beforeEach(() => {
    ...
    handler = new RolleBesetztEventHandler(mockAddEintragHandler, mockLogger);
  });

  describe('AC1: Handler sollte ETB-Eintrag für Rollenbesetzung erstellen', () => {
    it('should create ETB entry with correct text format "{vorname} {nachname} übernimmt Rolle {rollenName}"', async () => {
      // Given (Arrange)
      ...
      expect(receivedCommand.text).toBe('Max Mustermann übernimmt Rolle Leiter BHP');
    });

    it('should set kategorie to PERSONAL for rolle besetzung', async () => {
      // Given (Arrange)
      const event = createTestEvent();
    ...
    it('should pass besetztVon as userId to ETB command', async () => {
      // Given (Arrange)
      const besetztVon = generateTestCuid();
      const event = createTestEvent({ besetztVon });
      ...
      // Then (Assert)
      const receivedCommand = mockAddEintragHandler.execute.mock.calls[0]?.[0]!;
      expect(receivedCommand.userId).toBe(besetztVon);
    });
  ...
  describe('AC2: Handler sollte bei erfolgreicher Erstellung Logger.log() aufrufen', () => {
    it('should call Logger.log() at start of processing', async () => {
      ...
      // Then (Assert)
      expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining('Creating ETB entry for RolleBesetzt'), 'RolleBesetztEventHandler');
    });
    ...
    it('should call Logger.log() with success message on successful creation', async () => {
      ...
      // Then (Assert)
      expect(mockLogger.log).toHaveBeenCalledWith(expect.stringMatching(/ETB entry created for RolleBesetzt.*rolle=Leiter BHP/), 'RolleBesetztEventHandler');
    });
  ...
  describe('AC3: Fire-and-Forget - Handler sollte bei Command-Erstellung-Fehler Logger.error() aufrufen', () => {
    it('should continue processing when AddEintragCommand.create() succeeds with empty names (defensive)', async () => {
      ...
      // Hinweis: AddEintragCommand.create() akzeptiert technisch "  übernimmt Rolle ..."
  ...
  describe('AC4: Fire-and-Forget - Handler sollte bei Handler-Fehler Logger.error() aufrufen', () => {
    it('should call Logger.error() when AddEintragHandler.execute() returns failure', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringMatching(/Failed to add ETB entry for RolleBesetzt.*Repository save failed.*severity=ERROR.*actionRequired=Manual ETB entry may be needed/),
  ...
  describe('AC5: Fire-and-Forget - Handler sollte bei unerwartetem Fehler Logger.error() aufrufen', () => {
    it('should call Logger.error() when AddEintragHandler.execute() throws', async () => {
      ...
      expect(errorCall[1]).toBe('RolleBesetztEventHandler');
    });
    ...
    it('should include stack trace in error log', async () => {
      ...
      expect(errorLogCall[1]).toBe('RolleBesetztEventHandler');
    });
    ...
    it('should handle non-Error thrown objects gracefully', async () => {
      ...
      // Then (Assert)
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringMatching(/CRITICAL.*String error.*severity=CRITICAL.*actionRequired=Manual ETB entry may be needed/), 'RolleBesetztEventHandler');
    });
  ...
  describe('AC7: Validation - Handler sollte fehlende Snapshot-Daten loggen', () => {
    it('should call Logger.warn() when personVorname is missing', async () => {
      ...
      // Then (Assert)
      expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringMatching(/RolleBesetzt event has missing snapshot data.*vorname=.*nachname=Mustermann.*rolle=LNA/), 'RolleBesetztEventHandler');
    });
    ...
    it('should call Logger.warn() when personNachname is missing', async () => {
      ...
      // Then (Assert)
      expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringMatching(/RolleBesetzt event has missing snapshot data.*vorname=Max.*nachname=/), 'RolleBesetztEventHandler');
    });
    ...
    it('should call Logger.warn() when rollenName is missing', async () => {
      ...
      // Then (Assert)
      expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringMatching(/RolleBesetzt event has missing snapshot data.*rolle=/), 'RolleBesetztEventHandler');
    });
    ...
    it('should call Logger.warn() when all snapshot data is missing', async () => {
      ...
      // Then (Assert)
      expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringMatching(/RolleBesetzt event has missing snapshot data/), 'RolleBesetztEventHandler');
    });
  ...
  describe('Event Data Extraction', () => {
    it('should extract all event properties correctly', async () => {
      ...
      expect(receivedCommand.userId).toBe(specificEvent.besetztVon);
  ...
  describe('ETB Entry Text Format Validation', () => {
    it('should validate exact format "{vorname} {nachname} übernimmt Rolle {rollenName}"', async () => {
      // Given (Arrange)
      const event = createTestEvent({
        personVorname: 'Test',
        personNachname: 'User',
      ...
      // Then (Assert)
      const receivedCommand = mockAddEintragHandler.execute.mock.calls[0]?.[0]!;
      const expectedText = 'Test User übernimmt Rolle LNA';
      expect(receivedCommand.text).toBe(expectedText);

      // Validate format structure
      expect(receivedCommand.text).toMatch(/^.+ .+ übernimmt Rolle .+$/);
    });
  });
