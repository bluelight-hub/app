...
export class ErinnerungIntensiviertEventHandler implements IEventHandler<ErinnerungIntensiviertEvent> {
  ...
  /**
   * Verarbeitet ErinnerungIntensiviertEvent.
   *
  ...
  async handle(event: ErinnerungIntensiviertEvent): Promise<void> {
    this.logger.log(
      `[STUB] ErinnerungIntensiviertEvent received: einsatzId=${event.einsatzId}, erinnerungId=${event.erinnerungId}, intensiviertAm=${event.intensiviertAm.toISOString()}`,
      'ErinnerungIntensiviertEventHandler',
    );

    // Story 5.1: Hier wird ETB-Eintrag mit Kategorie ERINNERUNG erstellt
    // Template: ERINNERUNG_ETB_TEMPLATES.INTENSIVIERT
  }
}
