...
/**
 * Type-Safe EntityId für StammFahrzeug Aggregate.
 *
 ...
 * einem Type Alias (`type StammFahrzeugId = string`) oder Branded Type:
 *
 * 1. **Compile-Time Type Safety:**
 *    - Verhindert versehentliches Vertauschen von IDs unterschiedlicher Aggregates
 *    - TypeScript unterscheidet `StammFahrzeugId` von `FahrzeugtypId`, `QualifikationId`, etc.
 ...
 *    - "Find Usages" findet alle Stellen die StammFahrzeugId verwenden
 ...
 * ```typescript
 * type StammFahrzeugId = string & { __brand: 'StammFahrzeugId' };
 * ```
 * Probleme:
 ...
 * die Base Class `EntityId<'StammFahrzeug'>` und bietet dafür Type Safety, Runtime
 * Validation und Konsistenz mit dem Rest der Domain Layer.
 *
 * @example
 * ```typescript
 * // Auto-Generation
 * const id = StammFahrzeugId.create();
 * if (id.isSuccess) {
 *   console.log(id.value.toString()); // "clw3h8x9y..."
 * }
 *
 * // Mit existierendem CUID
 * const id2 = StammFahrzeugId.create('clw3h8x9y0000qwertyuiopas');
 *
 * // Type-Safety (Compile-Time)
 * function processStammFahrzeug(id: StammFahrzeugId) { ... }
 * const fahrzeugtypId = FahrzeugtypId.create().value;
 * processStammFahrzeug(fahrzeugtypId); // ❌ Compile Error: FahrzeugtypId ≠ StammFahrzeugId
 *
 * // String wäre NICHT type-safe
 * function processStammFahrzeugUnsafe(id: string) { ... }
 * processStammFahrzeugUnsafe(fahrzeugtypId.value); // ✅ Kompiliert (GEFAHR!)
 * processStammFahrzeugUnsafe("invalid-id"); // ✅ Kompiliert (GEFAHR!)
 * ```
 */
export class StammFahrzeugId extends EntityId<'StammFahrzeug'> {}
