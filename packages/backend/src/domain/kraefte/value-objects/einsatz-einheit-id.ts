...
/**
 * Type-Safe EntityId für EinsatzEinheit Aggregate.
 *
 ...
 * einem Type Alias (`type EinsatzEinheitId = string`) oder Branded Type:
 *
 * 1. **Compile-Time Type Safety:**
 *    - Verhindert versehentliches Vertauschen von IDs unterschiedlicher Aggregates
 *    - TypeScript unterscheidet `EinsatzEinheitId` von `EinsatzPersonId`, `EinsatzId`, etc.
 ...
 * ```typescript
 * // Auto-Generation
 * const id = EinsatzEinheitId.create();
 * if (id.isSuccess) {
 *   console.log(id.value.toString()); // "clw3h8x9y..."
 * }
 *
 * // Mit existierendem CUID
 * const id2 = EinsatzEinheitId.create('clw3h8x9y0000qwertyuiopas');
 *
 * // Type-Safety (Compile-Time)
 * function processEinheit(id: EinsatzEinheitId) { ... }
 * const personId = EinsatzPersonId.create().value;
 * processEinheit(personId); // ❌ Compile Error: EinsatzPersonId ≠ EinsatzEinheitId
 * ```
 */
export class EinsatzEinheitId extends EntityId<'EinsatzEinheit'> {}
