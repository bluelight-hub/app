...
import { FuehrungsrhythmusTemplateAktualisiertEvent } from '../fuehrungsrhythmus-template-aktualisiert.event';
...
describe('FuehrungsrhythmusTemplateAktualisiertEvent', () => {
  let testTemplateId: FuehrungsrhythmusTemplateId;
  let testUserId: UserId;
  const testName = 'Standard Fuehrungsrhythmus';
  ...
  describe('Event Erstellung', () => {
    it('sollte Event mit allen Properties erstellen', () => {
      ...
      // When
      const event = new FuehrungsrhythmusTemplateAktualisiertEvent(testTemplateId, testName, testUserId, aggregateId);

      // Then
      expect(event.templateId).toBe(testTemplateId);
      expect(event.name).toBe(testName);
      expect(event.aktualisiertVon).toBe(testUserId);
    ...
    it('sollte Event ohne aggregateId erstellen', () => {
      // When
      const event = new FuehrungsrhythmusTemplateAktualisiertEvent(testTemplateId, testName, testUserId);
  ...
  describe('eventName', () => {
    it('sollte static eventName() "fuehrungsrhythmus-template.aktualisiert" zurueckgeben', () => {
      // Then
    ...
    it('sollte eventName via Constructor-Zugriff korrekt liefern', () => {
      ...
      // Then
      expect((event.constructor as typeof DomainEvent).eventName()).toBe('fuehrungsrhythmus-template.aktualisiert');
    });
  });
  ...
  describe('Immutabilitaet', () => {
    it('sollte alle Properties als readonly haben', () => {
      ...
      expect(event.aktualisiertVon).toBeDefined();
      expect(event.eventId).toBeDefined();
  ...
  describe('Value Object Typisierung', () => {
    it('sollte mit typed Value Objects arbeiten', () => {
      ...
      expect(event.aktualisiertVon).toBeInstanceOf(UserId);
    });
  });
  ...
  describe('Unique EventId Generation', () => {
    it('sollte einzigartige eventIds fuer verschiedene Events generieren', () => {
      // Given
      const event1 = new FuehrungsrhythmusTemplateAktualisiertEvent(testTemplateId, 'Template A', testUserId);
      const event2 = new FuehrungsrhythmusTemplateAktualisiertEvent(testTemplateId, 'Template B', testUserId);
  ...
  describe('Naming Convention', () => {
    it('sollte der Naming Convention folgen (namespace.action_past_tense)', () => {
      // Given
      const name = FuehrungsrhythmusTemplateAktualisiertEvent.eventName();
  ...
  describe('eventVersion', () => {
    it('sollte default Version 1 haben', () => {
      // Then
      expect(FuehrungsrhythmusTemplateAktualisiertEvent.eventVersion()).toBe(1);
    });
  });
});
