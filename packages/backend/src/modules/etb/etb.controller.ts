...
export class EtbController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Gefilterte Liste von ETB-Einträgen',
    type: EtbEntriesResponse,
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Filter: vonZeitstempel',
    schema: { type: 'string', format: 'date-time' },
  })
  @ApiResponse({
    status: 200,
    description: 'Filter: bisZeitstempel',
    schema: { type: 'string', format: 'date-time' },
  })
  ...
  async findOne(@Param('id') id: string): Promise<EtbEntry> {
  ...
  @ApiResponse({
    status: 400,
    description: 'Ungültige Eingabedaten oder Eintrag bereits abgeschlossen',
  })
  @ApiResponse({ status: 404, description: 'ETB-Eintrag wurde nicht gefunden' })
  async update(@Param('id') id: string, @Body() updateEtbDto: UpdateEtbDto): Promise<EtbEntry> {
  ...
  @ApiResponse({ status: 404, description: 'ETB-Eintrag wurde nicht gefunden' })
  async closeEntry(@Param('id') id: string, @Req() req: RequestWithUser): Promise<EtbEntry> {
  ...
  async addAttachment(
    ...
    const created = await this.etbService.addAttachment(id, file, addAttachmentDto);
  ...
  @ApiResponse({ status: 404, description: 'ETB-Eintrag nicht gefunden' })
  async findAttachments(@Param('id') id: string): Promise<EtbAttachment[]> {
  ...
  async findAttachment(@Param('id') id: string): Promise<EtbAttachment> {
  ...
  async ueberschreibeEintrag(
    ...
    const result = await this.etbService.ueberschreibeEintrag(
      id,
      ueberschreibeEtbDto,
      userId,
      userName,
