...
export class SchemaValidator {
    ...
    validate(jsonData: any, config: ImportConfig = {}): ValidationResult {
        ...
        try {
            ...
            if (format === 'unknown') {
                result.errors.push({
                    code: 'UNKNOWN_FORMAT',
                    message: 'Unbekanntes JSON-Format. Muss entweder vollständiges oder vereinfachtes Seed-Format sein.',
                    severity: 'error'
                });
                return result;
            }
            ...
            // Schema-Validierung
            const isValid = format === 'full'
                ? this.fullValidator(jsonData)
                : this.simpleValidator(jsonData);
     ...
     */
    private validateSemantics(data: any, format: 'full' | 'simple', config: ImportConfig): {
        errors: ValidationError[];
        warnings: ValidationWarning[];
    } {
        ...
        if (format === 'full') {
            ...
            const duplicateNames = einsatzNames.filter((name: string, index: number) =>
                einsatzNames.indexOf(name) !== index
            );
            ...
            data.einsaetze.forEach((einsatz: any, einsatzIndex: number) => {
                if (einsatz.etbEntries && einsatz.etbEntries.length > 0) {
                    this.validateEtbEntries(einsatz.etbEntries, `einsaetze[${einsatzIndex}]`, errors, warnings);
                }
            });
        }
    ...
    private validateEtbEntries(
        ...
        // Duplikate prüfen
        const duplicates = laufendeNummern.filter((num, index) =>
            laufendeNummern.indexOf(num) !== index
        );
    ...
    private validateCommonSemantics(
        ...
        if (format === 'full') {
            const totalEtbEntries = data.einsaetze.reduce((sum: number, einsatz: any) =>
    ...
    private generateMetadata(data: any, format: 'full' | 'simple'): ValidationResult['metadata'] {
        ...
        if (format === 'full') {
            ...
            metadata.etbEntriesCount = data.einsaetze.reduce((sum: number, einsatz: any) =>
                sum + (einsatz.etbEntries?.length || 0), 0
            );
        } else {
 ...
 */
export function validateSeedDataString(jsonString: string, config: ImportConfig = {}): ValidationResult {
    try {
