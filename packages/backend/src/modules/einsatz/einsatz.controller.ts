    ...
    async create(@Body() createEinsatzDto: CreateEinsatzDto): Promise<Einsatz> {
        ...
        if (environment === 'production') {
            ...
            if (enableEinsatzCreation !== 'true') {
                this.logger.warn(`Einsatzerstellung in Production verweigert für: ${createEinsatzDto.name}`);
                throw new ForbiddenException('Einsatzerstellung ist in der Produktionsumgebung nicht aktiviert');
            }

            // Zusätzliche Production-Validierung
            if (createEinsatzDto.name?.toLowerCase().includes('test') ||
                createEinsatzDto.name?.toLowerCase().includes('dev')) {
                this.logger.warn(`Production-Einsatz mit Test/Dev Namen verweigert: ${createEinsatzDto.name}`);
                throw new ForbiddenException('Test- oder Entwicklungsnamen sind in der Produktionsumgebung nicht erlaubt');
            }
        }
