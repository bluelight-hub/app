...
@Command({
  name: 'seed:einsatz',
  description:
    'Erstellt einen Test-Einsatz mit dem angegebenen Namen oder aus einem vordefinierten DRK-Profil',
})
export class SeedEinsatzCommand extends CommandRunner {
   ...
   */
  async run(_passedParams: string[], options?: SeedEinsatzOptions): Promise<void> {
    try {
  ...
  @Option({
    ...
    description:
      'Vordefiniertes DRK-Profil verwenden (manv, sanitaetsdienst, katastrophenschutz, betreuung, rettungsdienst, psnv, ausbildung)',
  })
  ...
  @Option({
    ...
    description:
      'Profile nach Kategorie filtern (rettungsdienst, katastrophenschutz, sanitaetsdienst, betreuung, psnv, manv, ausbildung)',
  })
  ...
  private showProfileList(): void {
    ...
    profiles.forEach((profile) => {
      ...
      this.logger.log(
        `   Kategorie: ${profile.metadata.category} | Priorität: ${profile.metadata.priority}`,
      );
      this.logger.log(
        `   Personen: ${profile.metadata.estimatedPersonsAffected} | Dauer: ${profile.metadata.estimatedDurationHours}h`,
      );
      this.logger.log('');
    });
