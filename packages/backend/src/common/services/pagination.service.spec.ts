...
describe('PaginationService', () => {
  ...
  describe('paginate', () => {
    ...
    it('sollte mit komplexen Abfrageoptionen korrekt umgehen', async () => {
      ...
      const options = {
        where: {
          OR: [{ name: { contains: 'User' } }, { email: { contains: 'example.com' } }],
        },
        orderBy: { name: 'asc' },
        include: {
          posts: true,
    ...
    it('sollte einen Fehler werfen, wenn das Modell nicht existiert', async () => {
      ...
      await expect(paginationService.paginate('nonExistingModel', {}, 1, 10)).rejects.toThrow(
        "Prisma model 'nonExistingModel' not found",
      );
    });
